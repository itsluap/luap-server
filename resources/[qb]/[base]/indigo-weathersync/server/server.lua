...
local function nextWeatherStage()
    ...
    TriggerEvent("indigo-weathersync:server:RequestStateSync")
end
...
-- EVENTS
RegisterNetEvent('indigo-weathersync:server:RequestStateSync', function()
    TriggerClientEvent('indigo-weathersync:client:SyncWeather', -1, CurrentWeather, blackout)
    TriggerClientEvent('indigo-weathersync:client:SyncTime', -1, baseTime, timeOffset, freezeTime)
end)

RegisterNetEvent('indigo-weathersync:server:setWeather', function(weather)
...
RegisterNetEvent('indigo-weathersync:server:setTime', function(hour, minute)
    local src = getSource(source)
    if isAllowedToChange(src) then
...
RegisterNetEvent('indigo-weathersync:server:toggleBlackout', function(state)
    local src = getSource(source)
    if isAllowedToChange(src) then
...
RegisterNetEvent('indigo-weathersync:server:toggleFreezeTime', function(state)
    local src = getSource(source)
    if isAllowedToChange(src) then
...
RegisterNetEvent('indigo-weathersync:server:toggleDynamicWeather', function(state)
    local src = getSource(source)
    if isAllowedToChange(src) then
