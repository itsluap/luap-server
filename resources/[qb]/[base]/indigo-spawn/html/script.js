...
$(document).on('click', '#IconClassStyle', function(evt){
    ...
    if (type !== "lab") {
        ...
        $.post('https://indigo-spawn/setCam', JSON.stringify({
            posname: location,
            type: type,
        }));
...
$(document).on('click', '.GreenBTN', function(evt){
    ...
    if (Mainlocation !== null){
        ...
        if (MainspawnType == "apartment1") {
            $.post('https://indigo-spawn/spawnplayerappartment1', JSON.stringify({
                spawnloc: Mainlocation,
                apartName: apartNames,
            }));
        } else if(MainspawnType !== "appartment"){
            $.post('https://indigo-spawn/spawnplayer', JSON.stringify({
                spawnloc: Mainlocation,
                typeLoc: MainspawnType
            }));
        }else {
            $.post('https://indigo-spawn/chooseAppa', JSON.stringify({
...
function CloseAddCoord() {
    $.post('https://indigo-spawn/CloseAddCoord', JSON.stringify({}));
