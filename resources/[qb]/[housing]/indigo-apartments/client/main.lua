    ...
    QBCore.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            QBCore.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                local data = exports['indigo-interior']:CreateApartmentFurnished(coords)
                Wait(100)
                HouseObj = data[1]
                POIOffsets = data[2]
                InApartment = true
        ...
        if new ~= nil then
            if new then
                TriggerEvent('indigo-interior:client:SetNewState', true)
            else
                TriggerEvent('indigo-interior:client:SetNewState', false)
            end
        else
            TriggerEvent('indigo-interior:client:SetNewState', false)
        end
...
local function LeaveApartment(house)
    TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_open", 0.1)
    openHouseAnim()
    TriggerServerEvent("indigo-apartments:returnBucket")
    DoScreenFadeOut(500)
    ...
    exports['indigo-interior']:DespawnInterior(HouseObj, function()
        ...
        TriggerServerEvent('indigo-apartments:server:SetInsideMeta', CurrentApartment, false)
...
RegisterNetEvent('indigo-apartments:client:LastLocationHouse', function(apartmentType, apartmentId)
