...
local function showEntranceHeaderMenu()
    ...
    if not isOwned then
        headerMenu[#headerMenu+1] = {
            ...
            params = {
                event = "indigo-houses:client:ViewHouse",
                args = {}
            }
        }
    else
        if isOwned and HasHouseKey then
            headerMenu[#headerMenu+1] = {
                ...
                params = {
                    event = "indigo-houses:client:EnterHouse",
                    args = {}
                }
            }
            headerMenu[#headerMenu+1] = {
        ...
        elseif isOwned and not HasHouseKey then
            headerMenu[#headerMenu+1] = {
                ...
                params = {
                    event = "indigo-houses:client:RequestRing",
                    args = {}
                }
            }
            headerMenu[#headerMenu+1] = {
            ...
            if QBCore.Functions.GetPlayerData().job and GlobalState.isLEO then
                headerMenu[#headerMenu+1] = {
...
local function showExitHeaderMenu()
    ...
    headerMenu[#headerMenu+1] = {
        ...
        params = {
            event = "indigo-houses:client:ExitOwnedHouse",
            args = {}
        }
    }
    if isOwned then
        headerMenu[#headerMenu+1] = {
            ...
            params = {
                event = "indigo-houses:client:FrontDoorCam",
                args = {}
            }
        }
        headerMenu[#headerMenu+1] = {
            ...
            params = {
                event = "indigo-houses:client:AnswerDoorbell",
                args = {}
            }
        }
    end
...
RegisterNetEvent('indigo-houses:client:setHouseConfig', function(houseConfig)
...
local function SetClosestHouse()
    ...
    if not IsInside then
        ...
        if ClosestHouse ~= nil and tonumber(dist) < 30 then
            QBCore.Functions.TriggerCallback('indigo-houses:server:ProximityKO', function(key, owned)
                HasHouseKey = key
                isOwned = owned
            end, ClosestHouse)
        end
    end
    TriggerEvent('indigo-garages:client:setHouseGarage', ClosestHouse, HasHouseKey)
end

local function setHouseLocations()
    if ClosestHouse ~= nil then
        QBCore.Functions.TriggerCallback('indigo-houses:server:getHouseLocations', function(result)
            if result ~= nil then
...
local function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		QBCore.Functions.TriggerCallback('indigo-houses:server:getHouseDecorations', function(result)
...
local function RemoveHouseKey(citizenData)
    TriggerServerEvent('indigo-houses:server:removeHouseKey', ClosestHouse, citizenData)
    CloseMenuFull()
end
...
local function getKeyHolders()
    ...
    local p = promise.new()
    QBCore.Functions.TriggerCallback('indigo-houses:server:getHouseKeyHolders', function(holders)
        p:resolve(holders)
    end,ClosestHouse)
    ...
    if holders == nil or next(holders) == nil then
        ...
        for k, _ in pairs(holders) do
            keyholderMenu[#keyholderMenu+1] = {
                ...
                params = {
                    event = "indigo-houses:client:OpenClientOptions",
                    args = {
    ...
    keyholderOptions = {
        {
            ...
            params = {
                event = "indigo-houses:client:RevokeKey",
                args = {
...
local function getDataForHouseTier(house, coords)
    if Config.Houses[house].tier == 1 then
        return exports['indigo-interior']:CreateApartmentShell(coords)
    elseif Config.Houses[house].tier == 2 then
        return exports['indigo-interior']:CreateTier1House(coords)
    elseif Config.Houses[house].tier == 3 then
        return exports['indigo-interior']:CreateTrevorsShell(coords)
    elseif Config.Houses[house].tier == 4 then
        return exports['indigo-interior']:CreateCaravanShell(coords)
    elseif Config.Houses[house].tier == 5 then
        return exports['indigo-interior']:CreateLesterShell(coords)
    elseif Config.Houses[house].tier == 6 then
        return exports['indigo-interior']:CreateRanchShell(coords)
    elseif Config.Houses[house].tier == 7 then
        return exports['indigo-interior']:CreateContainer(coords)
    elseif Config.Houses[house].tier == 8 then
        return exports['indigo-interior']:CreateFurniMid(coords)
    elseif Config.Houses[house].tier == 9 then
        return exports['indigo-interior']:CreateFurniMotelModern(coords)
    elseif Config.Houses[house].tier == 10 then
        return exports['indigo-interior']:CreateFranklinAunt(coords)
    elseif Config.Houses[house].tier == 11 then
        return exports['indigo-interior']:CreateGarageMed(coords)
    elseif Config.Houses[house].tier == 12 then
        return exports['indigo-interior']:CreateMichael(coords)
    elseif Config.Houses[house].tier == 13 then
        return exports['indigo-interior']:CreateOffice1(coords)
    elseif Config.Houses[house].tier == 14 then
        return exports['indigo-interior']:CreateStore1(coords)
    elseif Config.Houses[house].tier == 15 then
        return exports['indigo-interior']:CreateWarehouse1(coords)
    elseif Config.Houses[house].tier == 16 then
        return exports['indigo-interior']:CreateFurniMotelStandard(coords) -- End of free shells
    elseif Config.Houses[house].tier == 17 then
        return exports['indigo-interior']:CreateMedium2(coords)
    elseif Config.Houses[house].tier == 18 then
        return exports['indigo-interior']:CreateMedium3(coords)
    elseif Config.Houses[house].tier == 19 then
        return exports['indigo-interior']:CreateBanham(coords)
    elseif Config.Houses[house].tier == 20 then
        return exports['indigo-interior']:CreateWestons(coords)
    elseif Config.Houses[house].tier == 21 then
        return exports['indigo-interior']:CreateWestons2(coords)
    elseif Config.Houses[house].tier == 22 then
        return exports['indigo-interior']:CreateClassicHouse(coords)
    elseif Config.Houses[house].tier == 23 then
        return exports['indigo-interior']:CreateClassicHouse2(coords)
    elseif Config.Houses[house].tier == 24 then
        return exports['indigo-interior']:CreateClassicHouse3(coords)
    elseif Config.Houses[house].tier == 25 then
        return exports['indigo-interior']:CreateHighend1(coords)
    elseif Config.Houses[house].tier == 26 then
        return exports['indigo-interior']:CreateHighend2(coords)
    elseif Config.Houses[house].tier == 27 then
        return exports['indigo-interior']:CreateHighend3(coords)
    elseif Config.Houses[house].tier == 28 then
        return exports['indigo-interior']:CreateHighend(coords)
    elseif Config.Houses[house].tier == 29 then
        return exports['indigo-interior']:CreateHighendV2(coords)
    elseif Config.Houses[house].tier == 30 then
        return exports['indigo-interior']:CreateStashHouse(coords)
    elseif Config.Houses[house].tier == 31 then
        return exports['indigo-interior']:CreateStashHouse2(coords)
    elseif Config.Houses[house].tier == 32 then
        return exports['indigo-interior']:CreateGarageLow(coords)
    elseif Config.Houses[house].tier == 33 then
        return exports['indigo-interior']:CreateGarageHigh(coords)
    elseif Config.Houses[house].tier == 34 then
        return exports['indigo-interior']:CreateOffice2(coords)
    elseif Config.Houses[house].tier == 35 then
        return exports['indigo-interior']:CreateOfficeBig(coords)
    elseif Config.Houses[house].tier == 36 then
        return exports['indigo-interior']:CreateBarber(coords)
    elseif Config.Houses[house].tier == 37 then
        return exports['indigo-interior']:CreateGunstore(coords)
    elseif Config.Houses[house].tier == 38 then
        return exports['indigo-interior']:CreateStore2(coords)
    elseif Config.Houses[house].tier == 39 then
        return exports['indigo-interior']:CreateStore3(coords)
    elseif Config.Houses[house].tier == 40 then
        return exports['indigo-interior']:CreateWarehouse2(coords)
    elseif Config.Houses[house].tier == 41 then
        return exports['indigo-interior']:CreateWarehouse3(coords)
    elseif Config.Houses[house].tier == 42 then
        return exports['indigo-interior']:CreateK4Coke(coords)
    elseif Config.Houses[house].tier == 43 then
        return exports['indigo-interior']:CreateK4Meth(coords)
    elseif Config.Houses[house].tier == 44 then
        return exports['indigo-interior']:CreateK4Weed(coords)
    elseif Config.Houses[house].tier == 45 then
        return exports['indigo-interior']:CreateContainer2(coords)
    elseif Config.Houses[house].tier == 46 then
        return exports['indigo-interior']:CreateFurniStash1(coords)
    elseif Config.Houses[house].tier == 47 then
        return exports['indigo-interior']:CreateFurniStash3(coords)
    elseif Config.Houses[house].tier == 48 then
        return exports['indigo-interior']:CreateFurniLow(coords)
    elseif Config.Houses[house].tier == 49 then
        return exports['indigo-interior']:CreateFurniMotel(coords)
    elseif Config.Houses[house].tier == 50 then
        return exports['indigo-interior']:CreateFurniMotelClassic(coords)
    elseif Config.Houses[house].tier == 51 then
        return exports['indigo-interior']:CreateFurniMotelHigh(coords)
    elseif Config.Houses[house].tier == 52 then
        return exports['indigo-interior']:CreateFurniMotelModern2(coords)
    elseif Config.Houses[house].tier == 53 then
        return exports['indigo-interior']:CreateFurniMotelModern3(coords)
    elseif Config.Houses[house].tier == 54 then
        return exports['indigo-interior']:CreateCoke(coords)
    elseif Config.Houses[house].tier == 55 then
        return exports['indigo-interior']:CreateCoke2(coords)
    elseif Config.Houses[house].tier == 56 then
        return exports['indigo-interior']:CreateMeth(coords)
    elseif Config.Houses[house].tier == 57 then
        return exports['indigo-interior']:CreateWeed(coords)
    elseif Config.Houses[house].tier == 58 then
        return exports['indigo-interior']:CreateWeed2(coords)
    elseif Config.Houses[house].tier == 59 then
        return exports['indigo-interior']:CreateMansion(coords)
    elseif Config.Houses[house].tier == 60 then
        return exports['indigo-interior']:CreateMansion2(coords)
    elseif Config.Houses[house].tier == 61 then
        return exports['indigo-interior']:CreateMansion3(coords)
    elseif Config.Houses[house].tier == 62 then
        return exports['indigo-interior']:CreateHotel1(coords)
    elseif Config.Houses[house].tier == 63 then
        return exports['indigo-interior']:CreateHotel2(coords)
    elseif Config.Houses[house].tier == 64 then
        return exports['indigo-interior']:CreateHotel3(coords)
    elseif Config.Houses[house].tier == 65 then
        return exports['indigo-interior']:CreateMotel1(coords)
    elseif Config.Houses[house].tier == 66 then
        return exports['indigo-interior']:CreateMotel2(coords)
    elseif Config.Houses[house].tier == 67 then
        return exports['indigo-interior']:CreateMotel3(coords)
    elseif Config.Houses[house].tier == 68 then
        return exports['indigo-interior']:CreateV2Default1(coords)
    elseif Config.Houses[house].tier == 69 then
        return exports['indigo-interior']:CreateV2Default2(coords)
    elseif Config.Houses[house].tier == 70 then
        return exports['indigo-interior']:CreateV2Default3(coords)
    elseif Config.Houses[house].tier == 71 then
        return exports['indigo-interior']:CreateV2Default4(coords)
    elseif Config.Houses[house].tier == 72 then
        return exports['indigo-interior']:CreateV2Default5(coords)
    elseif Config.Houses[house].tier == 73 then
        return exports['indigo-interior']:CreateV2Default6(coords)
    elseif Config.Houses[house].tier == 74 then
        return exports['indigo-interior']:CreateV2Deluxe1(coords)
    elseif Config.Houses[house].tier == 75 then
        return exports['indigo-interior']:CreateV2Deluxe2(coords)
    elseif Config.Houses[house].tier == 76 then
        return exports['indigo-interior']:CreateV2Deluxe3(coords)
    elseif Config.Houses[house].tier == 77 then
        return exports['indigo-interior']:CreateV2HighEnd1(coords)
    elseif Config.Houses[house].tier == 78 then
        return exports['indigo-interior']:CreateV2HighEnd2(coords)
    elseif Config.Houses[house].tier == 79 then
        return exports['indigo-interior']:CreateV2HighEnd3(coords)
    elseif Config.Houses[house].tier == 80 then
        return exports['indigo-interior']:CreateV2Medium1(coords)
    elseif Config.Houses[house].tier == 81 then
        return exports['indigo-interior']:CreateV2Medium2(coords)
    elseif Config.Houses[house].tier == 82 then
        return exports['indigo-interior']:CreateV2Medium3(coords)
    elseif Config.Houses[house].tier == 83 then
        return exports['indigo-interior']:CreateV2Modern1(coords)
    elseif Config.Houses[house].tier == 84 then
        return exports['indigo-interior']:CreateV2Modern2(coords)
    elseif Config.Houses[house].tier == 85 then
        return exports['indigo-interior']:CreateV2Modern3(coords)
    else
...
local function LeaveHouse(house)
    if not FrontCam then
        ...
        exports['indigo-interior']:DespawnInterior(houseObj, function()
            ...
            TriggerServerEvent('indigo-houses:server:SetInsideMeta', house, false)
            CurrentHouse = nil
...
RegisterNetEvent('indigo-houses:server:sethousedecorations', function(house, decorations)
...
RegisterNetEvent('indigo-houses:client:sellHouse', function()
    if ClosestHouse and HasHouseKey then
        TriggerServerEvent('indigo-houses:server:viewHouse', ClosestHouse)
    end
end)

RegisterNetEvent('indigo-houses:client:EnterHouse', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
...
RegisterNetEvent('indigo-houses:client:RequestRing', function()
    if ClosestHouse ~= nil then
        TriggerServerEvent('indigo-houses:server:RingDoor', ClosestHouse)
    end
end)

AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent('indigo-houses:server:setHouses')
    SetClosestHouse()
    TriggerEvent('indigo-houses:client:setupHouseBlips')
    if Config.UnownedBlips then TriggerEvent('indigo-houses:client:setupHouseBlips2') end
    Wait(100)
    TriggerEvent('indigo-garages:client:setHouseGarage', ClosestHouse, HasHouseKey)
    TriggerServerEvent("indigo-houses:server:setHouses")
end)
...
RegisterNetEvent('indigo-houses:client:lockHouse', function(bool, house)
    Config.Houses[house].locked = bool
end)

RegisterNetEvent('indigo-houses:client:createHouses', function(price, tier)
    ...
    street = street:gsub("%-", " ")
    TriggerServerEvent('indigo-houses:server:addNewHouse', street, coords, price, tier)
    if Config.UnownedBlips then TriggerServerEvent('indigo-houses:server:createBlip') end
end)

RegisterNetEvent('indigo-houses:client:addGarage', function()
    if ClosestHouse ~= nil then
        local pos = GetEntityCoords(PlayerPedId())
        local heading = GetEntityHeading(PlayerPedId())
        local coords = {
        ...
        TriggerServerEvent('indigo-houses:server:addGarage', ClosestHouse, coords)
    else
        QBCore.Functions.Notify(Lang:t("error.no_house"), "error")
    end
end)

RegisterNetEvent('indigo-houses:client:toggleDoorlock', function()
    local pos = GetEntityCoords(PlayerPedId())
    local dist = #(pos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
    if dist <= 50.0 then
        if HasHouseKey then
            if Config.Houses[ClosestHouse].locked then
                TriggerServerEvent('indigo-houses:server:lockHouse', false, ClosestHouse)
                QBCore.Functions.Notify(Lang:t("success.unlocked"), "success", 2500)
            else
                TriggerServerEvent('indigo-houses:server:lockHouse', true, ClosestHouse)
...
RegisterNetEvent('indigo-houses:client:RingDoor', function(player, house)
...
RegisterNetEvent('indigo-houses:client:giveHouseKey', function()
    ...
    if player ~= -1 and distance < 2.5 and ClosestHouse ~= nil then
        ...
        if housedist < 10 then
            TriggerServerEvent('indigo-houses:server:giveHouseKey', playerId, ClosestHouse)
        else
...
RegisterNetEvent('indigo-houses:client:removeHouseKey', function()
    if ClosestHouse ~= nil then
        local pedpos = GetEntityCoords(PlayerPedId())
        local housedist = #(pedpos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
        if housedist <= 5 then
            QBCore.Functions.TriggerCallback('indigo-houses:server:getHouseOwner', function(result)
...
RegisterNetEvent('indigo-houses:client:RevokeKey', function(cData)
    RemoveHouseKey(cData.citizenData)
end)

RegisterNetEvent('indigo-houses:client:refreshHouse', function()
    Wait(100)
    SetClosestHouse()
end)

RegisterNetEvent('indigo-houses:client:SpawnInApartment', function(house)
    local pos = GetEntityCoords(PlayerPedId())
...
RegisterNetEvent('indigo-houses:client:enterOwnedHouse', function(house)
...
RegisterNetEvent('indigo-houses:client:LastLocationHouse', function(houseId)
    QBCore.Functions.GetPlayerData(function(PlayerData)
		if PlayerData.metadata["injail"] == 0 then
...
RegisterNetEvent('indigo-houses:client:setupHouseBlips', function() -- Setup owned on load
    CreateThread(function()
        Wait(2000)
        if LocalPlayer.state['isLoggedIn'] then
            QBCore.Functions.TriggerCallback('indigo-houses:server:getOwnedHouses', function(ownedHouses)
                if ownedHouses then
...
RegisterNetEvent('indigo-houses:client:setupHouseBlips2', function() -- Setup unowned on load
...
RegisterNetEvent('indigo-houses:client:createBlip', function(coords) -- Create unowned on command
...
RegisterNetEvent('indigo-houses:client:refreshBlips', function() -- Refresh unowned on buy
    for _, v in pairs(UnownedHouseBlips) do RemoveBlip(v) end
    Wait(250)
    TriggerEvent('indigo-houses:client:setupHouseBlips2')
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('indigo-houses:client:SetClosestHouse', function()
    SetClosestHouse()
end)

RegisterNetEvent('indigo-houses:client:viewHouse', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
...
RegisterNetEvent('indigo-houses:client:setLocation', function(cData)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    ...
    if IsInside then
        if HasHouseKey then
            if cData.id == 'setstash' then
                TriggerServerEvent('indigo-houses:server:setLocation', coords, ClosestHouse, 1)
            elseif cData.id == 'setoutift' then
                TriggerServerEvent('indigo-houses:server:setLocation', coords, ClosestHouse, 2)
            elseif cData.id == 'setlogout' then
                TriggerServerEvent('indigo-houses:server:setLocation', coords, ClosestHouse, 3)
            end
        else
            QBCore.Functions.Notify(Lang:t("error.not_owner"), "error")
        end
...
RegisterNetEvent('indigo-houses:client:refreshLocations', function(house, location, type)
...
RegisterNetEvent('indigo-houses:client:HomeInvasion', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
                        ...
                        if not Config.Houses[ClosestHouse].IsRaming then
                            ...
                            Skillbar.Start({
                                ...
                                if RamsDone + 1 >= Config.RamsNeeded then
                                    ...
                                    TriggerServerEvent('indigo-houses:server:SetHouseRammed', true, ClosestHouse)
                                    TriggerServerEvent('indigo-houses:server:SetRamState', false, ClosestHouse)
                            ...
                            TriggerServerEvent('indigo-houses:server:SetRamState', true, ClosestHouse)
                        else
...
RegisterNetEvent('indigo-houses:client:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('indigo-houses:client:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('indigo-houses:client:ResetHouse', function()
    if ClosestHouse ~= nil then
        if Config.Houses[ClosestHouse].IsRammed == nil then
            Config.Houses[ClosestHouse].IsRammed = false
            TriggerServerEvent('indigo-houses:server:SetHouseRammed', false, ClosestHouse)
            TriggerServerEvent('indigo-houses:server:SetRamState', false, ClosestHouse)
        end
...
RegisterNetEvent('indigo-houses:client:ExitOwnedHouse', function()
...
RegisterNetEvent('indigo-houses:client:FrontDoorCam', function()
    local door = vector3(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z)
...
RegisterNetEvent('indigo-houses:client:AnswerDoorbell', function()
    ...
    if CheckDistance(door, 3.5) and CurrentDoorBell ~= 0 then
        TriggerServerEvent("indigo-houses:server:OpenDoor", CurrentDoorBell, ClosestHouse)
        CurrentDoorBell = 0
    end
end)

RegisterNetEvent('indigo-houses:client:OpenStash', function()
...
RegisterNetEvent('indigo-houses:client:ChangeCharacter', function()
    ...
    if CheckDistance(stashLoc, 1.5) then
        ...
        exports['indigo-interior']:DespawnInterior(houseObj, function()
            ...
            InOwnedHouse = false
            IsInside = false
            TriggerServerEvent('indigo-houses:server:LogoutLocation')
        end)
    end
end)

RegisterNetEvent('indigo-houses:client:ChangeOutfit', function()
...
RegisterNetEvent('indigo-houses:client:ViewHouse', function()
...
RegisterNetEvent('indigo-houses:client:KeyholderOptions', function(cData)
...
RegisterNUICallback('HasEnoughMoney', function(cData, cb)
    QBCore.Functions.TriggerCallback('indigo-houses:server:HasEnoughMoney', function(_)
        cb('ok')
...
RegisterNUICallback('buy', function(_, cb)
    ...
    if Config.UnownedBlips then TriggerEvent('indigo-houses:client:refreshBlips') end
    TriggerServerEvent('indigo-houses:server:buyHouse', ClosestHouse)
    cb("ok")
end)
        ...
        if IsInside then
            ...
            if isInsideStashTarget then
                ...
                if IsControlJustPressed(0, 38) then
                    TriggerEvent('indigo-houses:client:OpenStash')
                    exports['ps-ui']:HideText()
                end
            end
            ...
            if isInsideOutfitsTarget then
                wait = 0
                if IsControlJustPressed(0, 38) then
                    TriggerEvent('indigo-houses:client:ChangeOutfit')
                    exports['ps-ui']:HideText()
                end
            end
            ...
            if isInsiteCharactersTarget then
                wait = 0
                if IsControlJustPressed(0, 38) then
                    TriggerEvent('indigo-houses:client:ChangeCharacter')
                    exports['ps-ui']:HideText()
                end
            end
        end
