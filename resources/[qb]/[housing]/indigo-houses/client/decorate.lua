...
local function SaveDecorations()
	if ClosestHouse then
		...
		TriggerServerEvent("indigo-houses:server:savedecorations", ClosestHouse, ObjectList)
	end
end
...
RegisterNetEvent('indigo-houses:client:decorate', function()
	Wait(500)
	if IsInside then
		if HasHouseKey then
...
RegisterNUICallback("buySelectedObject", function(data, cb)
    QBCore.Functions.TriggerCallback('indigo-houses:server:buyFurniture', function(isSuccess)
        if isSuccess then
            SetNuiFocus(false, false)
            cursorEnabled = false
            SaveDecorations()
