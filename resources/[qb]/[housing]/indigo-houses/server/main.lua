...
CreateThread(function()
    ...
    TriggerClientEvent("indigo-houses:client:setHouseConfig", -1, Config.Houses)
end)
...
QBCore.Commands.Add("decorate", Lang:t("info.decorate_interior"), {}, false, function(source)
    local src = source
    TriggerClientEvent("indigo-houses:client:decorate", src)
end)
...
QBCore.Commands.Add("createhouse", Lang:t("info.create_house"), {{name = "price", help = Lang:t("info.price_of_house")}, {name = "tier", help = Lang:t("info.tier_number")}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "realestate" then
        TriggerClientEvent("indigo-houses:client:createHouses", src, price, tier)
    else
...
QBCore.Commands.Add("addgarage", Lang:t('info.add_garage'), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "realestate" then
        TriggerClientEvent("indigo-houses:client:addGarage", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.realestate_only"), "error")
    end
end)
...
QBCore.Commands.Add("ring", Lang:t("info.ring_doorbell"), {}, false, function(source)
    local src = source
    TriggerClientEvent('indigo-houses:client:RequestRing', src)
end)
...
QBCore.Functions.CreateUseableItem("police_stormram", function(source, _)
    ...
    if (GlobalState.isLEO and Player.PlayerData.job.onduty) then
        TriggerClientEvent("indigo-houses:client:HomeInvasion", source)
    else
...
RegisterNetEvent('indigo-houses:server:setHouses', function()
    local src = source
    TriggerClientEvent("indigo-houses:client:setHouseConfig", src, Config.Houses)
end)

RegisterNetEvent('indigo-houses:server:createBlip', function()
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    TriggerClientEvent("indigo-houses:client:createBlip", -1, coords)
end)

RegisterNetEvent('indigo-houses:server:addNewHouse', function(street, coords, price, tier)
    local src = source
    street = street:gsub("%'", "")
...
RegisterNetEvent('indigo-houses:server:addGarage', function(house, coords)
    local src = source
...
RegisterNetEvent('indigo-houses:server:viewHouse', function(house)
    local src = source
    ...
    TriggerClientEvent('indigo-houses:client:viewHouse', src, houseprice, brokerfee, bankfee, taxes,
        pData.PlayerData.charinfo.firstname, pData.PlayerData.charinfo.lastname)
end)

RegisterNetEvent('indigo-houses:server:buyHouse', function(house)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('indigo-houses:server:lockHouse', function(bool, house)
    TriggerClientEvent('indigo-houses:client:lockHouse', -1, bool, house)
end)

RegisterNetEvent('indigo-houses:server:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    TriggerClientEvent('indigo-houses:server:SetRamState', -1, bool, house)
end)

RegisterNetEvent('indigo-houses:server:giveKey', function(house, target)
...
RegisterNetEvent('indigo-houses:server:removeHouseKey', function(house, citizenData)
    local src = source
    local newHolders = {}
    if housekeyholders[house] then
...
RegisterNetEvent('indigo-houses:server:OpenDoor', function(target, house)
    local OtherPlayer = QBCore.Functions.GetPlayer(target)
    if OtherPlayer then
        TriggerClientEvent('indigo-houses:client:SpawnInApartment', OtherPlayer.PlayerData.source, house)
    end
end)

RegisterNetEvent('indigo-houses:server:RingDoor', function(house)
    local src = source
    TriggerClientEvent('indigo-houses:client:RingDoor', -1, src, house)
end)

RegisterNetEvent('indigo-houses:server:savedecorations', function(house, decorations)
    MySQL.update('UPDATE player_houses SET decorations = ? WHERE house = ?', {json.encode(decorations), house})
    TriggerClientEvent("indigo-houses:server:sethousedecorations", -1, house, decorations)
end)

RegisterNetEvent('indigo-houses:server:LogoutLocation', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('indigo-multichar:logout', src)
end)

RegisterNetEvent('indigo-houses:server:giveHouseKey', function(target, house)
    local src = source
    ...
    if tPlayer then
        if housekeyholders[house] then
            ...
            MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', {json.encode(housekeyholders[house]), house})
            TriggerClientEvent('indigo-houses:client:refreshHouse', tPlayer.PlayerData.source)
...
RegisterNetEvent('indigo-houses:server:setLocation', function(coords, house, type)
    if type == 1 then
    ...
    TriggerClientEvent('indigo-houses:client:refreshLocations', -1, house, json.encode(coords), type)
end)

RegisterNetEvent('indigo-houses:server:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    TriggerClientEvent('indigo-houses:client:SetHouseRammed', -1, bool, house)
end)

RegisterNetEvent('indigo-houses:server:SetInsideMeta', function(insideId, bool)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('indigo-houses:server:buyFurniture', function(source, cb, price)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    local bankBalance = pData.PlayerData.money["bank"]
...
QBCore.Functions.CreateCallback('indigo-houses:server:ProximityKO', function(source, cb, house)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local retvalK = false
    local retvalO
...
QBCore.Functions.CreateCallback('indigo-houses:server:hasKey', function(source, cb, house)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local retval = false
    if Player then
...
QBCore.Functions.CreateCallback('indigo-houses:server:isOwned', function(source, cb, house)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('indigo-houses:server:getHouseOwner', function(_, cb, house)
    cb(houseownercid[house])
end)

QBCore.Functions.CreateCallback('indigo-houses:server:getHouseKeyHolders', function(source, cb, house)
    local retval = {}
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if housekeyholders[house] then
...
QBCore.Functions.CreateCallback('indigo-houses:server:getHouseDecorations', function(_, cb, house)
    local retval = nil
...
QBCore.Functions.CreateCallback('indigo-houses:server:getHouseLocations', function(_, cb, house)
    local retval = nil
    local result = MySQL.query.await('SELECT * FROM player_houses WHERE house = ?', {house})
    if result[1] then
        retval = result[1]
...
QBCore.Functions.CreateCallback('indigo-houses:server:getOwnedHouses', function(source, cb)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    if pData then
...
QBCore.Functions.CreateCallback('indigo-houses:server:getSavedOutfits', function(source, cb)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
