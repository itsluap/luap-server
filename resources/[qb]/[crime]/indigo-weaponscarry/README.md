...
# How to install
Step 3 and 4 are skippable if you do not use indigo-apartments or dont use indigo-apartments with routing buckets

1. Download the latest version of the script
2. Extract the files to your server root directory
3. Head over to your indigo-apartments and add this `exports['indigo-weaponscarry']:toggleProps()` to line 256 right under Wait(250) in the function EnterApartment
4. Now scroll down till you find `local function LeaveApartment` and past this right under it around line 317 `exports['indigo-weaponscarry']:toggleProps()`
5. Add the following line to your server.cfg file `ensure indigo-weaponscarry`
6. ENJOY!

# Additional Information // Exports

1. `exports["indigo-weaponscarry"]:GetPlayerCarryItems()` - Returns a table of all the items the player is carrying

2. `exports["indigo-weaponscarry"]:toggleProps()` - Toggles the props for the player and wont load them untill they are toggled back on OR if they are in their apartments

3. `exports["indigo-weaponscarry"]:refreshProps()` - Refreshes the props for the player toggle this at at the END of your refreshskin events to make sure the props get removed and refreshed the proper way

4. `exports["indigo-weaponscarry"]:isCarryingObject()` - Returns true if the player is carrying an any item

5. `exports["indigo-weaponscarry"]:makeObjectBusy(item, toggle)` - This acts as if the particular item was removed or used by the player, this is useful for items that can be used multiple times such as a fishing rod, this would allow the player to when they start using the rod for it to be removed on the back untill they are done using it

6. `exports["indigo-weaponscarry"]:carryProp(item)` - Acts as if the player was given x item to carry, this DOES NOT work with weapons on back ONLY CARRYABLE items

7. `exports["indigo-weaponscarry"]:removeProp(item)` - Acts as if x item was removed from the player, this DOES NOT work with weapons on back ONLY CARRYABLE items

8. `exports["indigo-weaponscarry"]:isCarryingAnObject(item)` - Returns true if the players is carrying a specific item
