...
local function RegisterTraphouseEntranceTarget(traphouseID, traphouseData)
    ...
    exports['indigo-target']:AddBoxZone(boxName, coords, boxData.length, boxData.width, {
        ...
        options = {
            {
                type = 'client',
                event = 'indigo-traphouse:client:EnterTraphouse',
...
local function RegisterTraphouseInteractionTarget(traphouseID, traphouseData)
    ...
    local options = {
        {
            type = "client",
            event = "indigo-traphouse:client:target:TakeOver",
    ...
    if IsKeyHolder then
        options = {
            {
                type = "client",
                event = "indigo-traphouse:client:target:ViewInventory",
                label = Lang:t("targetInfo.inventory"),
                traphouseData = traphouseData
            },
            {
                type = "client",
                event = "indigo-traphouse:client:target:TakeMoney",
        ...
        if IsHouseOwner then
            options[#options+1] = {
                type = "client",
                event = "indigo-traphouse:client:target:SeePinCode",
...
local function RegisterTraphouseExitTarget(coords, traphouseID, traphouseData)
    ...
    exports['indigo-target']:AddBoxZone(boxName, coords, boxData.length, boxData.width, {
        ...
        options = {
            {
                type = "client",
                event = "indigo-traphouse:client:target:ExitTraphouse",
...
local function OpenHeaderMenu(data)
    ...
    headerMenu[#headerMenu+1] = {
        ...
        params = {
            event = "indigo-traphouse:client:target:CloseMenu",
        }
    }
...
local function EnterTraphouse(data)
    ...
    data = exports['indigo-interior']:CreateTrevorsShell(coords)
    TraphouseObj = data[1]
    POIOffsets = data[2]
...
RegisterNetEvent('indigo-traphouse:client:EnterTraphouse', function()
    if ClosestTraphouse ~= nil then
...
RegisterNetEvent('indigo-traphouse:client:TakeoverHouse', function(TraphouseId)
    QBCore.Functions.Progressbar("takeover_traphouse", Lang:t("info.taking_over"), math.random(1000, 3000), false, true, {
        ...
        TriggerServerEvent('indigo-traphouse:server:AddHouseKeyHolder', PlayerData.citizenid, TraphouseId, true)
    end, function()
        QBCore.Functions.Notify(Lang:t("error.cancelled"), "error")
    end)
end)

RegisterNetEvent('indigo-traphouse:client:target:ViewInventory', function (data)
...
RegisterNetEvent('indigo-traphouse:client:target:TakeOver', function ()
    TriggerServerEvent('indigo-traphouse:server:TakeoverHouse', CurrentTraphouse)
end)

RegisterNetEvent('indigo-traphouse:client:target:TakeMoney', function ()
    TriggerServerEvent("indigo-traphouse:server:TakeMoney", CurrentTraphouse)
end)

RegisterNetEvent('indigo-traphouse:client:target:SeePinCode', function (data)
    QBCore.Functions.Notify(Lang:t('info.pin_code', { value = data.traphouseData.pincode }))
end)

RegisterNetEvent('indigo-traphouse:client:target:ExitTraphouse', function (data)
    LeaveTraphouse(data.traphouseID, Config.TrapHouses[data.traphouseID])
end)

RegisterNetEvent('indigo-traphouse:client:SyncData', function(k, data)
...
RegisterNetEvent('indigo-traphouse:client:target:CloseMenu', function ()
                                    ...
                                    if not IsEntityDead(targetPed) then
                                        if CanRob then
                                            if InDistance then
                                                ...
                                                TriggerServerEvent('indigo-traphouse:server:RobNpc', ClosestTraphouse)
                                                CanRob = false
                                            end
                                        end
                                    end
