                        ...
                        for k = 1, searches do
                            if app.bannedJobs[k] == playerJob then
                                if hadApp(app.app) then
                                    TriggerServerEvent('indigo-laptop:server:LostAccess', app.app)
                                end
                                goto skip
...
RegisterNetEvent('indigo-laptop:client:openlaptop', function()
    if haveItem(Config.LaptopDevice) then
        SetDisplay(true)
    end
end)

RegisterNetEvent('indigo-laptop:client:CustomNotification', function(text, icon, color, iconbg, length)
    if haveItem(Config.LaptopDevice) then
        SendNUIMessage({
            action = "custom-notif",
            data = {
...
RegisterNUICallback('laptop/checkout', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('indigo-laptop:server:checkout', function(result)
        if result == "bank" then
            cb({
                status = 'error',
...
RegisterNUICallback('setting/save', function(data, cb)
    ...
    cb("ok")
    TriggerServerEvent("indigo-laptop:server:settings:set", data["setting"])
end)
