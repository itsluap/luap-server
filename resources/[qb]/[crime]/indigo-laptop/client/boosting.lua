...
local function DelayDelivery()
    SetTimeout(math.random(10, 30 * 1000), function()
        TriggerServerEvent('indigo-laptop:server:FinalDestination')
    end)
end
        ...
        CreateThread(function()
            while State and State.boostHacks > 0 do
                ...
                if DoesEntityExist(car) then
                    ...
                    TriggerServerEvent('indigo-laptop:server:SyncBlips', pos, NetID)
                else
                    checks = checks + 1
                    if checks >= 3 and not DoesEntityExist(car) then
                        TriggerServerEvent('indigo-laptop:server:CancelBoost', NetID, Plate)
            ...
            if DoesEntityExist(car) then
                TriggerServerEvent('indigo-laptop:server:SyncBlips', nil, NetID)
        ...
        QBCore.Functions.Progressbar('checking_vin', 'Checking VIN', 10000, false, true,
            { -- Name | Label | Time | useWhileDead | canCancel
                ...
                if not Entity(entity).state.vinchecked then
                    ...
                    QBCore.Functions.TriggerCallback('indigo-laptop:server:checkVin', function(cb)
                        res:resolve(cb)
                    end, NetworkID)
    ...
    if DoesEntityExist(car) then
        ...
        if dist <= 3.5 then -- 2.5 is the distance in qbcore vehiclekeys if you use more or less then please edit this.
            if #(vector3(carCoords.x, carCoords.y, carCoords.z) - GetEntityCoords(car)) <= 6.9 then
                AntiSpam = true
                TriggerServerEvent('indigo-laptop:server:SpawnPed')
...
RegisterNetEvent('indigo-laptop:client:MissionStarted',
...
RegisterNUICallback('boosting/start', function(data, cb)
    QBCore.Functions.TriggerCallback('indigo-laptop:server:CanStartBoosting', function(result)
        print(result)
        if result == "success" then
            --TriggerServerEvent('indigo-laptop:server:StartBoosting', data.id, currentCops)
            cb({
                status = 'success',
...
local function DeliverCar()
    ...
    TriggerServerEvent('indigo-laptop:server:finishBoost', NetID)
...
RegisterNetEvent('indigo-laptop:client:ReturnCar', function(coords)
...
RegisterNetEvent("indigo-laptop:client:ToVinScratch", function(coords)
    local function Inside()
        VehicleCheck(true)
    end
    CZone = lib.zones.sphere({
...
RegisterNetEvent('indigo-laptop:client:finishContract', function(table)
    if CZone then
        CZone:remove()
        CZone = nil
    end
...
RegisterNetEvent('indigo-laptop:client:HackCar', function()
    ...
    if haveItem(Config.Boosting.HackingDevice) then
        if cache.vehicle then
            ...
            if State and State.boostHacks > 0 and not State.boostCooldown then
                ...
                TriggerServerEvent('indigo-laptop:server:SyncPlates', success)
...
if Config.Boosting.Debug then
    RegisterCommand('hackcar', function()
        ...
        TriggerEvent('indigo-laptop:client:HackCar')
    end, false)
end
...
RegisterNetEvent('indigo-laptop:client:SpawnPeds', function(netIds, Location)
    for i = 1, #netIds do
        -- Get the Ped --
...
RegisterNetEvent('indigo-laptop:client:SyncBlips', function(coords, newNet)
...
RegisterNetEvent('indigo-laptop:client:ContractHandler', function(table, currentdate)
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.TriggerCallback('indigo-laptop:server:GetContracts', function(result)
        Contracts = result
...
RegisterNUICallback('boosting/deny', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('indigo-laptop:server:DeclineContract', function(result)
        if result == "success" then
            cb({
                status = 'success',
...
RegisterNUICallback('boosting/transfer', function(data, cb)
    ...
    local contract = data.contractID
    QBCore.Functions.TriggerCallback('indigo-laptop:server:TransferContracts', function(result)
        if result == "success" then
            cb({
                status = 'success',
...
RegisterNUICallback("boosting/cancel", function(data, cb)
    TriggerServerEvent('indigo-laptop:server:CancelBoost', NetID, nil, true)
    cb("success")
end)
...
RegisterNUICallback('boosting/queue', function(data, cb)
    QBCore.Functions.TriggerCallback('indigo-laptop:server:joinQueue', function(result)
        if result == "success" then
            if data.status then
                cb({
                    status = 'success',
...
RegisterNetEvent('indigo-laptop:client:QueueHandler', function(value)
    inQueue = value
end)
