...
RegisterNetEvent('indigo-laptop:server:crateOpened', function(crateID)
    local _s = source
    QBCore.Functions.GetPlayer(_s).Functions.RemoveItem('drill', 1)
    crates[crateID].isOpened = true
    TriggerClientEvent('indigo-laptop:client:updateCrates', -1, crates)
end)

QBCore.Functions.CreateCallback('indigo-laptop:server:getCrateStatus', function(source, cb, crateNetID, crateentity)
