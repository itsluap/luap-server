...
QBCore.Functions.CreateUseableItem(Config.LaptopDevice, function(source, item)
    ...
    if Player.Functions.GetItemByName(Config.LaptopDevice) then
        TriggerClientEvent('indigo-laptop:client:openlaptop', source)
    end
end)
...
RegisterNetEvent('indigo-laptop:server:LostAccess', function(app)
    local src = source
    if app == "boosting" then
        TriggerEvent("indigo-laptop:server:QuitQueue", src)
    end
end)


RegisterNetEvent('indigo-laptop:server:settings:set', function(setting)
    local src = source
...
RegisterNetEvent('indigo-laptop:server:RemoveItem', function(item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
