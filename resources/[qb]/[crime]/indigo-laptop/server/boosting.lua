...
RegisterNetEvent('indigo-laptop:server:FinalDestination', function()
    local src = source
    ...
    if currentRuns[CID] and not currentRuns[CID].dropOff and currentRuns[CID].type == "boosting" then
        ...
        TriggerClientEvent('indigo-laptop:client:ReturnCar', src, place)
    ...
    elseif currentRuns[CID] and not currentRuns[CID].dropOff and currentRuns[CID].type == "vinscratch" then
        ...
        TriggerClientEvent("indigo-laptop:client:ToVinScratch", src, place)
...
local function SpawnCar(src)
    ...
    if DoesEntityExist(car) then
        ...
        if GetResourceState('ox_fuel') == "started" then
            ...
            TriggerClientEvent('indigo-laptop:client:setvehicleFuel', src, car)
        end
        SetVehicleNumberPlateText(car, plate)
        currentRuns[CID].NetID = NetworkGetNetworkIdFromEntity(car)
        TriggerClientEvent('indigo-laptop:client:MissionStarted', src, currentRuns[CID].NetID, coords, plate)
...
QBCore.Functions.CreateCallback('indigo-laptop:server:CanStartBoosting', function(source, cb, cops, id)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if location then
        ...
        if SpawnCar(src) then
            ...
            TriggerClientEvent('indigo-laptop:client:ContractHandler', src, currentContracts[CID])
...
RegisterNetEvent('indigo-laptop:server:SpawnPed', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local CID = Player.PlayerData.citizenid
    ...
    TriggerClientEvent('indigo-laptop:client:SpawnPeds', src, netIds, currentRuns[CID].Location)
    DeletePeds(netIds)
end)
...
RegisterNetEvent('indigo-laptop:server:SyncBlips', function(coords, NetID)
    local car = NetworkGetEntityFromNetworkId(NetID)
    local state = Entity(car).state.Boosting

    if not state then return end

    TriggerClientEvent('indigo-laptop:client:SyncBlips', -1, coords, NetID)
end)
...
RegisterNetEvent('indigo-laptop:server:SyncPlates', function(success)
    local src = source
...
QBCore.Functions.CreateUseableItem(Config.Boosting.HackingDevice, function(source, _)
    ...
    if Player.Functions.GetItemByName(Config.Boosting.HackingDevice) then
        TriggerClientEvent('indigo-laptop:client:HackCar', source)
    end
end)

----- ** EVERYTHING TO DO QUEUE ** -----

QBCore.Functions.CreateCallback('indigo-laptop:server:joinQueue', function(source, cb, status)
    print(status)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return cb("error") end
    ...
    if LookingForContracts[CID].online then
        TriggerClientEvent('indigo-laptop:client:QueueHandler', src, LookingForContracts[CID].active)
    end
end)


RegisterNetEvent('indigo-laptop:server:QuitQueue', function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('indigo-laptop:server:fuckvin', function(netid, model, mods)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('indigo-laptop:server:finishBoost', function(netId, isvin)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local CID = Player.PlayerData.citizenid
    ...
    currentRuns[CID] = nil
    TriggerClientEvent('indigo-laptop:client:finishContract', src, currentContracts[CID])
end)
...
RegisterNetEvent('indigo-laptop:server:CancelBoost', function(netId, Plate, b)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local CID = Player.PlayerData.citizenid
    if not Plate then
...
-- ** CONTRACTS ** --
RegisterNetEvent('indigo-laptop:server:DeclineBoost', function(contract)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
QBCore.Functions.CreateCallback('indigo-laptop:server:DeclineContract', function(source, cb, contract)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return cb("error") end
...
QBCore.Functions.CreateCallback('indigo-laptop:server:checkVin', function(_, cb, NetID)
    local entity = NetworkGetEntityFromNetworkId(NetID)
...
QBCore.Functions.CreateCallback('indigo-laptop:server:TransferContracts', function(source, cb, playerID, contractID)
    local src = source
    ...
    TriggerClientEvent('indigo-laptop:client:ContractHandler', Reciever.PlayerData.source, currentContracts[RecieveCID])
    cb("success")
end)
...
QBCore.Functions.CreateCallback('indigo-laptop:server:GetContracts', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    local CID = Player.PlayerData.citizenid
    if not currentContracts[CID] then currentContracts[CID] = {} end
...
local function generateContract(src, contract, vehicle, mission)
    ...
    if contract and vehicle and mission then
        ...
        TriggerClientEvent('indigo-laptop:client:ContractHandler', src, currentContracts[CID], GetCurrentTime())
    else
            ...
            for k, v in pairs(LookingForContracts) do
                if currentContracts[k] then
                    ...
                    elseif #currentContracts[k] >= Config.Boosting.MaxBoosts then
                        v.active = false
                        if v.online then
                            TriggerClientEvent('indigo-laptop:client:QueueHandler', v.src, false)
                        end
                    end
