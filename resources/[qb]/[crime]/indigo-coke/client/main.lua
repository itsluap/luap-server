...
Citizen.CreateThread(function()
	...
	while true do
		...
		elseif dist <= 3 and inUse then
			sleep = 0
			DrawText3D(coord.x, coord.y, coord.z, 'Someone has recently rented a plane. plesae wait a moment')
		else
			sleep = 3000
		end
		Citizen.Wait(sleep)
...
function PlaneSpawn()
    ...
    airplane = CreateVehicle(planehash, 2131.38, 4807.92, 41.2, 116.93, true, false)
    ...
    while true do
    	Citizen.Wait(1)
    	 DrawText3D(2131.38, 4807.92, 41.2, "Coke Plane")
		...
		while delivering do
			...
			if disttocoord <= 30 then
				...
				if IsControlJustPressed(1, 51) then
					...
					QBCore.Functions.Progressbar("picking_", "Picking up the delivery..", Config.pickupTime, false, true, {
