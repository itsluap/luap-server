        ...
        for key, value in pairs(Config.PawnLocation) do
            exports['qb-target']:AddBoxZone('PawnShop'..key, value.coords, value.length, value.width, {
                ...
                options = {
                    {
                        type = 'client',
                        event = 'indigo-pawnshop:client:openMenu',
                        icon = 'fas fa-ring',
                        label = 'Pawn Shop',
                    },
                },
...
RegisterNetEvent('indigo-pawnshop:client:openMenu', function()
        ...
        if GetClockHours() >= Config.TimeOpen and GetClockHours() <= Config.TimeClosed then
            local pawnShop = {
                ...
                },
                {
                    ...
                    params = {
                        event = 'indigo-pawnshop:client:openPawn',
                        args = {
            ...
            if not isMelting then
                pawnShop[#pawnShop + 1] = {
                    ...
                    params = {
                        event = 'indigo-pawnshop:client:openMelt',
                        args = {
            ...
            if canTake then
                pawnShop[#pawnShop + 1] = {
                    ...
                    params = {
                        isServer = true,
                        event = 'indigo-pawnshop:server:pickupMelted',
                        args = {
                            items = meltedItem
                        }
                    }
...
RegisterNetEvent('indigo-pawnshop:client:openPawn', function(data)
    QBCore.Functions.TriggerCallback('indigo-pawnshop:server:getInv', function(inventory)
            ...
            for i = 1, #data.items do
                if v.name == data.items[i].item then
                    pawnMenu[#pawnMenu + 1] = {
                        ...
                        params = {
                            event = 'indigo-pawnshop:client:pawnitems',
                            args = {
...
RegisterNetEvent('indigo-pawnshop:client:openMelt', function(data)
    QBCore.Functions.TriggerCallback('indigo-pawnshop:server:getInv', function(inventory)
        local PlyInv = inventory
        local meltMenu = {
            {
            ...
            for i = 1, #data.items do
                if v.name == data.items[i].item then
                    meltMenu[#meltMenu + 1] = {
                        ...
                        params = {
                            event = 'indigo-pawnshop:client:meltItems',
                            args = {
                                label = QBCore.Shared.Items[v.name].label,
...
RegisterNetEvent('indigo-pawnshop:client:pawnitems', function(item)
    ...
    if sellingItem then
        ...
        if tonumber(sellingItem.amount) > 0 then
            TriggerServerEvent('indigo-pawnshop:server:sellPawnItems', item.name, sellingItem.amount, item.price)
        else
    ...
    if meltingItem then
        ...
        if meltingItem.amount ~= nil then
            if tonumber(meltingItem.amount) > 0 then
                TriggerServerEvent('indigo-pawnshop:server:meltItemRemove', item.name, meltingItem.amount, item)
...
RegisterNetEvent('indigo-pawnshop:client:startMelting', function(item, meltingAmount, meltTimees)
    if not isMelting then
        isMelting = true
        meltTime = meltTimees
        meltedItem = {}
...
RegisterNetEvent('indigo-pawnshop:client:resetPickup', function()
    canTake = false
end)
