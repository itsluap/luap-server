...
local function exploitBan(id, reason)
    ...
    TriggerEvent('qb-log:server:CreateLog', 'pawnshop', 'Player Banned', 'red',
        string.format('%s was banned by %s for %s', GetPlayerName(id), 'indigo-pawnshop', reason), true)
    DropPlayer(id, 'You were permanently banned by the server for: Exploiting')
end

RegisterNetEvent('indigo-pawnshop:server:sellPawnItems', function(itemName, itemAmount, itemPrice)
    local src = source
    ...
    TriggerClientEvent('indigo-pawnshop:client:openMenu', src)
end)

RegisterNetEvent('indigo-pawnshop:server:meltItemRemove', function(itemName, itemAmount, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.Functions.RemoveItem(itemName, itemAmount) then
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[itemName], 'remove')
        local meltTime = (tonumber(itemAmount) * item.time)
        TriggerClientEvent('indigo-pawnshop:client:startMelting', src, item, tonumber(itemAmount), (meltTime * 60000 / 1000))
...
RegisterNetEvent('indigo-pawnshop:server:pickupMelted', function(item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local playerCoords = GetEntityCoords(GetPlayerPed(src))
    local dist
    ...
    for _, v in pairs(item.items) do
        ...
        for _, m in pairs(v.item.reward) do
            ...
            if Player.Functions.AddItem(m.item, (meltedAmount * rewardAmount)) then
                ...
                TriggerClientEvent('indigo-pawnshop:client:resetPickup', src) -- moving reset to try and combat duplication due to player having maxed inv (player is going to lose whatever they couldn't carry)
            else
                TriggerClientEvent('indigo-pawnshop:client:openMenu', src)
                return
            end
    ...
    TriggerClientEvent('indigo-pawnshop:client:resetPickup', src)
    TriggerClientEvent('indigo-pawnshop:client:openMenu', src)
end)

QBCore.Functions.CreateCallback('indigo-pawnshop:server:getInv', function(source, cb)
