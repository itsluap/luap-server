...
RegisterNetEvent('indigo-prison:client:Locker', function()
    if inJail then
...
RegisterNetEvent('indigo-prison:client:ForceOpenLocker', function(data)
    local prisonStash = QBCore.Functions.GetPlayerData().citizenid
    local locker = exports['indigo-input']:ShowInput({
    ...
    if locker then
        ...
        QBCore.Functions.TriggerCallback('indigo-prison:server:DoesStashExist', function(stashExist)
            if stashExist then
                QBCore.Functions.Progressbar('opening_prisonlocker', 'Opening the locker...', 2000, false, true, {
                    disableMovement = true,
                    disableCarMovement = true,
...
RegisterNetEvent('indigo-prison:client:RemoveLockers', function()
...
RegisterNetEvent('indigo-prison:client:SpawnLockers', function()
	    ...
	    for k, v in pairs(Config.GabzLockers) do
            ...
            exports['qb-target']:AddBoxZone("lockers"..k, v.coords, 1.5, 1.6, {
                ...
                options = {
                    {
                        type = "client",
                        event = "indigo-prison:client:Locker",
                    ...
                    {
                        type = "client",
                        event = "indigo-prison:client:ForceOpenLocker",
                        icon = "fas fa-box-open",
