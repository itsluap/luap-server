...
-------------------------------------------
------ ONLY USED FOR indigo-prisonJOBS --------
-------------------------------------------
	...
	if not targetsCreated then
		CheckTime = exports['indigo-target']:AddBoxZone("prisontime", vector3(1827.3, 2587.72, 46.01), 0.4, 0.55, {
			...
			maxZ = 47.01,
				...
				{
					type = "client",
					event = "indigo-prison:client:checkTime",
					icon = "fas fa-clock",
					label = "Check Jail Time",
				},
				{
					type = "client",
					event = "indigo-prison:client:jobMenu",
		...
		for k,v in pairs(Config.Locations.PrisonBreak) do
			exports['indigo-target']:AddBoxZone("prisonbreak"..k, vector3(v.coords.x, v.coords.y, v.coords.z), v.length, v.width, {
				...
				maxZ = v.coords.z + 1,
					...
					{
						type = "client",
						event = "indigo-prison:StartPrisonBreak",
		...
		Canteen = exports['indigo-target']:AddBoxZone("prisoncanteen", vector3(1780.95, 2560.05, 45.67), 3.8, 0.5, {
			...
			maxZ = 45.85,
				...
				{
					type = "client",
					event = "indigo-prison:client:useCanteen",
		...
		Slushy = exports['indigo-target']:AddBoxZone("prisonslushy", vector3(1777.64, 2559.97, 45.67), 0.5, 0.7, {
			...
			maxZ = 46.75,
				...
				{
					type = "client",
					event = "indigo-prison:client:slushy",
		...
		Coffee = exports['indigo-target']:AddBoxZone("prisoncoffee", vector3(1778.83, 2560.04, 45.67), 0.5, 0.3, {
			...
			maxZ = 46.75,
				...
				{
					type = "client",
					event = "indigo-prison:client:coffee",
					icon = "fas fa-mug-hot",
					label = "Make Coffee",
				},
			},
		...
		Soda = exports['indigo-target']:AddBoxZone("prisonsoda", vector3(1778.26, 2560.02, 45.67), 0.6, 0.55, {
			...
			maxZ = 46.75,
				...
				{
					type = "client",
					event = "indigo-prison:client:soda",
					icon = "fas fa-cash-register",
					label = "Make Soda",
				},
			},
		...
		if Config.Crafting then
			Crafting = exports['indigo-target']:AddBoxZone("prisoncrafting", vector3(Config.CraftingLocation.x, Config.CraftingLocation.y, Config.CraftingLocation.z), 1.4, 0.5, {
				...
				maxZ = Config.CraftingLocation.z + 1,
					...
					{
						type = "client",
						event = "indigo-prison:CraftingMenu",
						icon = "fas fa-toolbox",
		...
		if not Config.QB_PrisonJobs then
			exports['indigo-target']:AddBoxZone("PrisonChinUp", Config.Workout.Chinup.coords, 1, 2.0, {
				...
				options = {
					{
						type = "client",
						event = "indigo-prison:client:DoChinUp",
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
	...
	TriggerEvent('indigo-prison:client:SpawnLockers')
	PlayerJob = QBCore.Functions.GetPlayerData().job
end)
...
RegisterNetEvent('QBCore:Client:OnPlayerUnload', function()
	...
	DestroyAllTargets()
	TriggerEvent('indigo-prison:client:RemoveLockers')
end)
...
-- Job Menu
RegisterNetEvent('indigo-prison:client:jobMenu', function()
    if inJail then
		...
		pjobMenu = {
			...
			},
			{
				...
				params = {
					isServer = false,
					event = "indigo-prison:jobapplyElectrician",
				}
			},
			{
				header = "Cook",
				...
				params = {
					isServer = false,
					event = "indigo-prison:jobapplyCook",
				}
			},
			{
				header = "Janitor",
				...
				params = {
					isServer = false,
					event = "indigo-prison:jobapplyJanitor",
				}
			},
			{
				header = "Close Menu",
...
RegisterNetEvent('indigo-prison:jobapplyElectrician', function(args)
    ...
    if dist < 2 then
        ...
        if inJail then
			...
			TriggerEvent('indigo-prison:client:jobMenu')
		end
	else
...
RegisterNetEvent('indigo-prison:jobapplyCook', function(args)
	local pos = GetEntityCoords(PlayerPedId())
	inRange = false
...
RegisterNetEvent('indigo-prison:jobapplyJanitor', function(args)
	local pos = GetEntityCoords(PlayerPedId())
	inRange = false
...
-- Check Time
RegisterNetEvent('indigo-prison:client:checkTime', function()
	if inJail then
		local pos = GetEntityCoords(PlayerPedId())
...
------------------------------------
-- Use Canteen
RegisterNetEvent('indigo-prison:client:useCanteen', function()
	if inJail then
		local ShopItems = {}
...
-- Slushy Machine
RegisterNetEvent('indigo-prison:client:slushy', function()
	if inJail then
		Wait(1000)
...
-- Soda Machine
RegisterNetEvent('indigo-prison:client:soda', function()
	if inJail then
		Wait(1000)
		local ped = PlayerPedId()
...
RegisterNetEvent('indigo-prison:client:coffee', function()
	if inJail then
		local CoffeeItems = {}
...
RegisterNetEvent('indigo-prison:CraftingMenu', function()
...
local function CraftItem(item)
	...
	QBCore.Functions.Progressbar('crafting_item', 'Crafting '..sharedItems[craftedItem].label, 5000, false, false, {
		...
		TriggerServerEvent('indigo-prison:server:GetCraftedItem', Config.CraftingItems[item].receive)
		TriggerServerEvent('indigo-prison:server:RemoveCraftingItems', item)
...
RegisterNetEvent('indigo-prison:CraftItem', function(data)
	QBCore.Functions.TriggerCallback("indigo-prison:server:CraftingMaterials", function(hasMaterials)
