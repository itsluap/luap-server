...
QBCore.Functions.CreateCallback('indigo-drugs:server:cornerselling:getAvailableDrugs', function(source, cb)
    cb(getAvailableDrugs(source))
end)

RegisterNetEvent('indigo-drugs:server:giveStealItems', function(drugType, amount)
...
RegisterNetEvent('indigo-drugs:server:sellCornerDrugs', function(drugType, amount, price)
    local src = source
    ...
    if hasItem.amount >= amount then
        ...
        TriggerClientEvent('indigo-drugs:client:refreshAvailableDrugs', src, getAvailableDrugs(src))
    else
        TriggerClientEvent('indigo-drugs:client:cornerselling', src)
    end
end)

RegisterNetEvent('indigo-drugs:server:robCornerDrugs', function(drugType, amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local availableDrugs = getAvailableDrugs(src)
