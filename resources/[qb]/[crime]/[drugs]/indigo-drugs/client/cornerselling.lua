            ...
            options = {
                {
                    ...
                    action = function()
                        ...
                        TriggerServerEvent('indigo-drugs:server:giveStealItems', stealData.drugType, stealData.amount)
...
local function SellToPed(ped)
    ...
    if hasTarget then
        while pedDist < 1.5 and not IsPedDeadOrDying(ped) do
            ...
            if getRobbed <= Config.RobberyChance then
                TriggerServerEvent('indigo-drugs:server:robCornerDrugs', drugType, bagAmount)
...
-- Events
RegisterNetEvent('indigo-drugs:client:cornerselling', function()
    QBCore.Functions.TriggerCallback('indigo-drugs:server:cornerselling:getAvailableDrugs', function(result)
...
RegisterNetEvent('indigo-drugs:client:refreshAvailableDrugs', function(items)
