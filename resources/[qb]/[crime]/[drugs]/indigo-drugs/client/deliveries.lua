...
AddStateBagChangeHandler('isLoggedIn', nil, function(_, _, value)
    if value then
        QBCore.Functions.TriggerCallback('indigo-drugs:server:RequestConfig', function(DealerConfig)
...
local function RequestDelivery()
    if not waitingDelivery then
        ...
        TriggerServerEvent('indigo-drugs:server:giveDeliveryItems', waitingDelivery)
        SetTimeout(2000, function()
            TriggerServerEvent('indigo-phone:server:sendNewMail', {
                ...
                button = {
                    enabled = true,
                    buttonEvent = "indigo-drugs:client:setLocation",
...
local function DeliverStuff()
    if deliveryTimeout > 0 then
        ...
        TriggerServerEvent('indigo-drugs:server:successDelivery', activeDelivery, false)
    end
    deliveryTimeout = 0
end
...
RegisterNetEvent('indigo-drugs:client:RefreshDealers', function(DealerData)
    if not Config.UseTarget and dealerCombo then dealerCombo:destroy() end
...
RegisterNetEvent('indigo-drugs:client:updateDealerItems', function(itemData, amount)
    TriggerServerEvent('indigo-drugs:server:updateDealerItems', itemData, amount, currentDealer)
end)

RegisterNetEvent('indigo-drugs:client:setDealerItems', function(itemData, amount, dealer)
    Config.Dealers[dealer]["products"][itemData.slot].amount = Config.Dealers[dealer]["products"][itemData.slot].amount - amount
end)

RegisterNetEvent('indigo-drugs:client:setLocation', function(locationData)
...
RegisterNetEvent('indigo-drugs:client:sendDeliveryMail', function(type, deliveryData)
