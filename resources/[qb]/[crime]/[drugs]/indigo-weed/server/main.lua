...
QBCore.Functions.CreateCallback('indigo-weed:server:getBuildingPlants', function(_, cb, building)
...
RegisterNetEvent('indigo-weed:server:placePlant', function(coords, sort, currentHouse)
    ...
    TriggerClientEvent('indigo-weed:client:refreshHousePlants', -1, currentHouse)
end)

RegisterNetEvent('indigo-weed:server:removeDeathPlant', function(building, plantId)
    MySQL.query('DELETE FROM house_plants WHERE plantid = ? AND building = ?', {plantId, building})
    TriggerClientEvent('indigo-weed:client:refreshHousePlants', -1, building)
end)
...
CreateThread(function()
    while true do
        ...
        TriggerClientEvent('indigo-weed:client:refreshPlantStats', -1)
        Wait((60 * 1000) * 19.2)
    end
end)
...
QBCore.Functions.CreateUseableItem("weed_amnesia_seed", function(source, item)
    TriggerClientEvent('indigo-weed:client:placePlant', source, 'amnesia', item)
end)

QBCore.Functions.CreateUseableItem("weed_ak47_seed", function(source, item)
    TriggerClientEvent('indigo-weed:client:placePlant', source, 'ak47', item)
end)

QBCore.Functions.CreateUseableItem("weed_nutrition", function(source, item)
    TriggerClientEvent('indigo-weed:client:foodPlant', source, item)
end)

RegisterServerEvent('indigo-weed:server:removeSeed', function(itemslot, seed)
    local src = source
