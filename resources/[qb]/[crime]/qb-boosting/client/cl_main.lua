  ...
  if GlobalState.OnTheDropoffWay then
    ...
    elseif(vinstarted == false) then
      if(DisablerTimes < 4) then
        DisablerUsed = true
        local minigame = exports['ps-ui']:Scrambler(function(success) if success then DisablerTimes = DisablerTimes + 1 TriggerEvent('qb-phone:client:CustomNotification', "Tracking Disabler", DisablerTimes..' / '..'5', 'fas fa-code', '#ff3636', 15000) else print("fail") end end, "numeric", 30, 0)  
    ...
    if(DisablerTimes < 5) then
      DisablerUsed = true
      local minigame = exports['ps-ui']:Scrambler(function(success) if success then DisablerTimes = DisablerTimes + 1 TriggerEvent('qb-phone:client:CustomNotification', "Tracking Disabler", DisablerTimes..' / '..'5', 'fas fa-code', '#ff3636', 15000) else print("fail") end end, "greek", 30, 0)  
...
function HasItem(item)
  local hasitem = false
    QBCore.Functions.TriggerCallback(Config['CoreSettings']['QBCORE']["HasItem"], function(result)
        hasitem = result
    end, item)
    Citizen.Wait(500)
    return hasitem
...
AddEventHandler("boosting:client:UseComputer" , function()
  if CanUseComputer then
    QBCore.Functions.Progressbar("boosting_scratch", "Connection to network...", 5000, false, true, {
