...
local function ResetHouseStateTimer(house)
    ...
    SetTimeout(time, function()
        ...
        TriggerClientEvent('indigo-houserobbery:client:ResetHouseState', -1, house)
    end)
end

-- Callbacks

QBCore.Functions.CreateCallback('indigo-houserobbery:server:GetHouseConfig', function(_, cb)
    cb(Config.Houses)
end)

-- Events

RegisterNetEvent('indigo-houserobbery:server:SetBusyState', function(cabin, house, bool)
    Config.Houses[house]["furniture"][cabin]["isBusy"] = bool
    TriggerClientEvent('indigo-houserobbery:client:SetBusyState', -1, cabin, house, bool)
end)

RegisterNetEvent('indigo-houserobbery:server:enterHouse', function(house)
    local src = source
    if not Config.Houses[house]["opened"] then
        ResetHouseStateTimer(house)
        TriggerClientEvent('indigo-houserobbery:client:setHouseState', -1, house, true)
    end
    TriggerClientEvent('indigo-houserobbery:client:enterHouse', src, house)
    Config.Houses[house]["opened"] = true
end)

RegisterNetEvent('indigo-houserobbery:server:searchCabin', function(cabin, house)
    local src = source
    ...
    TriggerClientEvent('indigo-houserobbery:client:setCabinState', -1, house, cabin, true)
end)

RegisterNetEvent('indigo-houserobbery:server:removeAdvancedLockpick', function()
...
RegisterNetEvent('indigo-houserobbery:server:removeLockpick', function()
    local Player = QBCore.Functions.GetPlayer(source)
