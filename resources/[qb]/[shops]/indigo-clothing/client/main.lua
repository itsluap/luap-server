...
RegisterNetEvent('indigo-clothing:client:getOutfits', function(requiredJob, gradeLevel)
    local gender = "male"
    if QBCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
    QBCore.Functions.TriggerCallback('indigo-clothing:server:getOutfits', function(result)
        openMenu({
        ...
        for k, v in pairs(Config.ClothingRooms) do
            ...
            if v.isGang then
                action = function()
                    ...
                    TriggerEvent('indigo-clothing:client:getOutfits', v.requiredJob, gradeLevel)
                end
            else
                action = function()
                    customCamLocation = v.cameraLocation
            ...
            if inZone then
                ...
                elseif zoneName == 'tattoo' then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        TriggerEvent('indigo-clothing:client:getOutfits', clothingRoom.requiredJob, gradeLevel)
                    end
                end
            end
            Wait(sleep)
...
RegisterNetEvent('indigo-clothing:client:openOutfitMenu', function()
    QBCore.Functions.TriggerCallback('indigo-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data, cb)
    TriggerEvent('indigo-clothing:client:loadOutfit', data)
    cb('ok')
end)
...
RegisterNetEvent('indigo-clothing:client:openMenu')
AddEventHandler('indigo-clothing:client:openMenu', function()
    customCamLocation = nil
    openMenu({
...
RegisterNetEvent('indigo-clothing:client:reloadOutfits')
AddEventHandler('indigo-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
...
RegisterNUICallback('close', function(_, cb)
    ...
    disableCam()
    FreezeEntityPosition(PlayerPedId(), false)
    TriggerEvent('indigo-clothing:client:onMenuClose')
    cb('ok')
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('indigo-clothing:server:removeOutfit', data.outfitName, data.outfitId)
...
function SaveSkin()
    ...
    TriggerServerEvent("indigo-clothing:saveSkin", model, clothing)
end
...
AddEventHandler("qb-clothes:loadSkin", function(_, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('indigo-clothing:client:loadPlayerClothing', data, PlayerPedId())
    end)
end)

RegisterNetEvent('indigo-clothing:client:loadPlayerClothing')
AddEventHandler('indigo-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('indigo-clothing:client:loadOutfit')
AddEventHandler('indigo-clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
...
RegisterNetEvent("indigo-clothing:client:adjustfacewear")
AddEventHandler("indigo-clothing:client:adjustfacewear",function(type)
...
function reloadSkin(health, armor)
    ...
    TriggerServerEvent("indigo-clothing:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES - Event 2
