...
RegisterNetEvent('indigo-vehicleshop:server:addPlayer', function(citizenid, gameTime)
    financetimer[citizenid] = gameTime
end)

-- Deduct stored game time from player on logout
RegisterNetEvent('indigo-vehicleshop:server:removePlayer', function(citizenid)
...
-- Callbacks
QBCore.Functions.CreateCallback('indigo-vehicleshop:server:getVehicles', function(source, cb)
    local src = source
...
RegisterNetEvent('indigo-vehicleshop:server:deleteVehicle', function (netId)
    local vehicle = NetworkGetEntityFromNetworkId(netId)
    DeleteEntity(vehicle)
end)

-- Sync vehicle for other players
RegisterNetEvent('indigo-vehicleshop:server:swapVehicle', function(data)
    local src = source
    TriggerClientEvent('indigo-vehicleshop:client:swapVehicle', -1, data)
    Wait(1500)-- let new car spawn
    TriggerClientEvent('indigo-vehicleshop:client:homeMenu', src)-- reopen main menu
end)

-- Send customer for test drive
RegisterNetEvent('indigo-vehicleshop:server:customTestDrive', function(vehicle, playerid)
    local src = source
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target))) < 3 then
        TriggerClientEvent('indigo-vehicleshop:client:customTestDrive', target, vehicle)
    else
...
RegisterNetEvent('indigo-vehicleshop:server:financePayment', function(paymentAmount, vehData)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('indigo-vehicleshop:server:financePaymentFull', function(data)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    local cash = player.PlayerData.money['cash']
    local bank = player.PlayerData.money['bank']
...
RegisterNetEvent('indigo-vehicleshop:server:buyShowroomVehicle', function(vehicle)
    local src = source
    ...
    if cash > tonumber(vehiclePrice) then
        ...
        TriggerClientEvent('indigo-vehicleshop:client:buyShowroomVehicle', src, vehicle, plate)
...
RegisterNetEvent('indigo-vehicleshop:server:financeVehicle', function(downPayment, paymentAmount, vehicle)
    local src = source
    ...
    -- need to put a check if the vehicle is a boat or plane, then put it in another garage that fits the type
...
RegisterNetEvent('indigo-vehicleshop:server:sellShowroomVehicle', function(data, playerid)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target.PlayerData.source))) < 3 then
        ...
        if cash >= tonumber(vehiclePrice) then
            ...
            TriggerClientEvent('indigo-vehicleshop:client:buyShowroomVehicle', target.PlayerData.source, vehicle, plate)
            ...
            exports['Renwed-Banking']:addAccountMoney(player.PlayerData.job.name, vehiclePrice)
...
RegisterNetEvent('indigo-vehicleshop:server:sellfinanceVehicle', function(downPayment, paymentAmount, vehicle, playerid)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    local target = QBCore.Functions.GetPlayer(tonumber(playerid))
...
RegisterNetEvent('indigo-vehicleshop:server:checkFinance', function()
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
