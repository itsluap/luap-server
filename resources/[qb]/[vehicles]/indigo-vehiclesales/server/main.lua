...
QBCore.Functions.CreateCallback("indigo-vehiclesales:server:CheckModelName", function(_, cb, plate)
    if plate then
...
RegisterNetEvent('qb-occasions:server:buyVehicle', function(vehicleData)
    ...
    if result[1] and next(result[1]) then
        if Player.PlayerData.money.bank >= result[1].price then
            ...
            TriggerEvent('indigo-phone:server:sendNewMailToOffline', SellerCitizenId, {
