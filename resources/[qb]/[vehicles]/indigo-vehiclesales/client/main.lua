                ...
                if Config.UseTarget then
                    ...
                    EntityZones[i] = exports['qb-target']:AddTargetEntity(occasionVehicles[Zone][i].car, {
                        options = {
                            {
                                type = "client",
                                event = "indigo-vehiclesales:client:OpenContract",
                                icon = "fas fa-car",
...
local function SellData(data, model)
    QBCore.Functions.TriggerCallback("indigo-vehiclesales:server:CheckModelName",function(DataReturning)
        local vehicleData = {}
        ...
        while listen do
            if IsControlJustReleased(0, 38) then -- E
                if spot then
                    ...
                    TriggerEvent('indigo-vehiclesales:client:OpenContract', data)
                else
                    if IsPedInAnyVehicle(PlayerPedId(), false) then
                        listen = false
                        TriggerEvent('qb-occasions:client:MainMenu')
                        --TriggerEvent('indigo-vehiclesales:client:SellVehicle')
                    else
...
RegisterNetEvent('qb-occasions:client:SellBackCar', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        QBCore.Functions.TriggerCallback('indigo-garages:server:checkVehicleOwner', function(owned, balance)
            if owned then
                if balance < 1 then
...
RegisterNetEvent('indigo-vehiclesales:client:SellVehicle', function()
...
RegisterNetEvent('indigo-vehiclesales:client:OpenContract', function(data)
...
RegisterNetEvent('qb-occasions:client:MainMenu', function()
    local MainMenu = {
        ...
        },
        {
            ...
            params = {
                event = 'indigo-vehiclesales:client:SellVehicle',
            }
        },
        {
