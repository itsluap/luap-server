...
RegisterNetEvent("indigo-stancer:airsuspension")
AddEventHandler("indigo-stancer:airsuspension", function(vehicle,val,coords)
...
RegisterNUICallback('setvehicleheight', function(data, cb)
    ...
    if vehicle ~= nil and vehicle ~= 0 and not busyairsus then
		busyairsus = true
		TriggerServerEvent("indigo-stancer:airsuspension",VehToNet(vehicle), data.val, GetEntityCoords(vehicle))
    end
	cb(true)
end)
...
RegisterNetEvent("indigo-stancer:addstancerkit", function()
	local IsInVehicle = IsPedInAnyVehicle(PlayerPedId())
		if IsInVehicle then
			QBCore.Functions.Progressbar("Installing The stancerkit", "Installing The stancerkit", 5000, false, true, {
				...
				TriggerServerEvent("indigo-stancer:server:removestancer")
