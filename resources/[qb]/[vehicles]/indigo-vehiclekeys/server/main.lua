...
RegisterNetEvent('indigo-vehiclekeys:server:GiveVehicleKeys', function(receiver, plate)
    local giver = source
...
RegisterNetEvent('indigo-vehiclekeys:server:AcquireVehicleKeys', function(plate)
    local src = source
    GiveKeys(src, plate)
end)

RegisterNetEvent('indigo-vehiclekeys:server:breakLockpick', function(itemName)
...
RegisterNetEvent('indigo-vehiclekeys:server:setVehLockState', function(vehNetId, state)
...
QBCore.Functions.CreateCallback('indigo-vehiclekeys:server:checkPlayerOwned', function(_, cb, plate)
...
QBCore.Commands.Add("givekeys", Lang:t("addcom.givekeys"), {{name = Lang:t("addcom.givekeys_id"), help = Lang:t("addcom.givekeys_id_help")}}, false, function(source, args)
	local src = source
    TriggerClientEvent('indigo-vehiclekeys:client:GiveKeys', src, tonumber(args[1]))
end)
