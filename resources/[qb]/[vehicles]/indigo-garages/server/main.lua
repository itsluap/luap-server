...
QBCore.Functions.CreateCallback('qb-garage:server:spawnvehicle', function (source, cb, vehInfo, coords, warp)
    ...
    --print('setting mods on impound car')
         ...
         MySQL.query('SELECT * FROM player_vehicles WHERE plate = ?', {plate}, function(result)
            if result[1] then
                if not UseLoafHousing then
                    hasHouseKey = exports['indigo-houses']:hasKey(result[1].license, result[1].citizenid, garage)
                end
                if hasHouseKey then
                    cb(true)
                else
...
RegisterNetEvent('indigo-garages:server:UpdateOutsideVehicles', function(Vehicles)
    local src = source
...
RegisterNetEvent('indigo-garages:server:UpdateOutsideVehicles2', function(plate, vehicle)
    -- from real indigo-garages --
