...
local function PublicGarage(garageName, type)
    ...
    exports['indigo-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                event = "indigo-garages:client:GarageMenu",
                args = {
                    garageId = garageName,
                    garage = garage,
...
local function ParkOwnedVehicle(veh, garageName, vehLocation, plate)
    ...
    if plate then
        ...
        TriggerServerEvent('indigo-garages:server:UpdateOutsideVehicles', OutsideVehicles)
        TriggerServerEvent('indigo-garages:server:UpdateOutsideVehicles2', plate, nil)
    end
...
local function AddRadialParkingOption()
    ...
    if IsPedInAnyVehicle(Player) then
        MenuItemId = exports['indigo-radialmenu']:AddOption({
            ...
            icon = 'square-parking',
            type = 'client',
            event = 'indigo-garages:client:ParkVehicle',
            shouldClose = true
        }, MenuItemId)
    else
        MenuItemId = exports['indigo-radialmenu']:AddOption({
            ...
            icon = 'warehouse',
            type = 'client',
            event = 'indigo-garages:client:OpenMenu',
            shouldClose = true
        }, MenuItemId)
    end
end
...
function JobMenuGarage(garageName)
    ...
    for veh, label in pairs(vehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "",
            params = {
                event = "indigo-garages:client:TakeOutGarage",
                args = {
                    vehicleModel = veh,
                    garage = garage
                }
...
RegisterNetEvent("indigo-garages:client:GarageMenu", function(data)
            ...
            for k, v in pairs(result) do
                ...
                if type == "depot" then
                    MenuGarageOptions[#MenuGarageOptions+1] = {
                        ...
                        params = {
                            event = "indigo-garages:client:TakeOutDepot",
                            args = {
                                vehicle = v,
...
RegisterNetEvent('indigo-garages:client:TakeOutGarage', function(data, cb)
    ...
    if garageType == 'job' then
        Wait(1000)
        TriggerEvent('qb-admin:client:luapSaveCar')
    end
end)
...
RegisterNetEvent('indigo-garages:client:OpenMenu', function()
    ...
    elseif CurrentHouseGarage then
        TriggerServerEvent('indigo-garages:luap:OpenHouseGarageMenu', CurrentHouseGarage)
    end
end)

RegisterNetEvent('indigo-garages:client:ParkVehicle', function()
    local ped = PlayerPedId()
    local curVeh = GetVehiclePedIsIn(ped)
    ParkVehicle(curVeh)
end)

RegisterNetEvent('indigo-garages:client:ParkLastVehicle', function(parkingName)
    local ped = PlayerPedId()
...
RegisterNetEvent('indigo-garages:client:addHouseGarage', function(house, garageInfo)
