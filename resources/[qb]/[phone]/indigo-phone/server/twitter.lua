...
RegisterNetEvent('indigo-phone:server:DeleteTweet', function(tweetId)
    local src = source
    ...
    TriggerClientEvent('indigo-phone:client:UpdateTweets', -1, src, Tweets, true)
end)

RegisterNetEvent('indigo-phone:server:UpdateTweets', function(TweetData)
    local src = source
    ...
    MySQL.insert('INSERT INTO phone_tweets (citizenid, firstName, lastName, message, url, tweetid, type) VALUES (?, ?, ?, ?, ?, ?, ?)', {
        ...
        if id then
            ...
            TriggerClientEvent('indigo-phone:client:UpdateTweets', -1, src, Tweets, false)
        end
    end)
end)
...
local function AddNewTweet(TweetData)
    ...
    MySQL.insert('INSERT INTO phone_tweets (citizenid, firstName, lastName, message, url, tweetid, type) VALUES (?, ?, ?, ?, ?, ?, ?)', {
        ...
        if id then
            ...
            TriggerClientEvent('indigo-phone:client:UpdateTweets', -1, 0, Tweets, false)
        end
    end)
