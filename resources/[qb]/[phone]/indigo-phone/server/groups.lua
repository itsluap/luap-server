...
local function pNotifyGroup(group, header, msg, icon, colour, length)
    if not group or not EmploymentGroup[group] then return print("Group not found...") end
    for _, v in pairs(EmploymentGroup[group].members) do
        TriggerClientEvent('indigo-phone:client:CustomNotification', v.Player,
            header or "NO HEADER",
            msg or "NO MSG",
...
local function DestroyGroup(groupID)
    ...
    TriggerEvent("indigo-phone:server:GroupDeleted", groupID, members)

    EmploymentGroup[groupID] = nil
    TriggerClientEvent('indigo-phone:client:RefreshGroupsApp', -1, EmploymentGroup)
...
local function setJobStatus(groupID, status, stages)
    ...
    for i=1, #m do
        if m[i] then
            TriggerClientEvent("indigo-phone:client:AddGroupStage", m[i], status, stages)
        end
    end
...
local function resetJobStatus(groupID)
    ...
    for i=1, #m do
        if m[i] then
            TriggerClientEvent("indigo-phone:client:AddGroupStage", m[i], EmploymentGroup[groupID].status, EmploymentGroup[groupID].stage)
            TriggerClientEvent('indigo-phone:client:RefreshGroupsApp', m[i], EmploymentGroup, true)
        end
    end
...
RegisterNetEvent("indigo-phone:server:employment_checkJobStauts", function ()
    local src = source
    local checkStatus = GetGroupByMembers(src)
    if checkStatus then
        TriggerClientEvent('indigo-phone:client:showEmploymentPage', src)
    else
        TriggerClientEvent('indigo-phone:client:showEmploymentGroupPage', src)
    end
end)

RegisterNetEvent("indigo-phone:server:jobcenter_CreateJobGroup", function(data)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('indigo-phone:server:jobcenter_DeleteGroup', function(data)
    local src = source
...
QBCore.Functions.CreateCallback('indigo-phone:server:GetGroupsApp', function(_, cb)
    cb(EmploymentGroup)
end)

RegisterNetEvent('indigo-phone:server:jobcenter_JoinTheGroup', function(data)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('indigo-phone:server:getAllGroups', function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback('indigo-phone:server:jobcenter_CheckPlayerNames', function(_, cb, csn)
    local Names = {}
...
RegisterNetEvent('indigo-phone:server:jobcenter_leave_grouped', function(data)
    local src = source
