...
QBCore.Functions.CreateCallback('indigo-phone:server:GetVehicleSearchResults', function(_, cb, search)
...
QBCore.Functions.CreateCallback('indigo-phone:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData
    if plate then
...
RegisterNetEvent('indigo-phone:server:ClientScanPlate', function(plate)
    local src = source
    local vehicleData = {}
    if plate then
        local result = exports.oxmysql:executeSync('SELECT * FROM player_vehicles WHERE plate = ?', {plate})
