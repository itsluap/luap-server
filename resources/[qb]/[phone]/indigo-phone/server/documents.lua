...
RegisterNetEvent("indigo-phone:server:sendDocument", function(data)
    local src = source
    ...
    if Ply.PlayerData.citizenid ~= Receiver.PlayerData.citizenid then
        ...
        TriggerClientEvent("indigo-phone:client:sendingDocumentRequest", data.StateID, data, Receiver, Ply, SenderName)
    else
        TriggerClientEvent('indigo-phone:client:CustomNotification', src,
            "Documents",
...
RegisterNetEvent("indigo-phone:server:sendDocumentLocal", function(data, playerId)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent("indigo-phone:client:sendingDocumentRequest", playerId, data, Receiver, Ply, SenderName)
end)
...
RegisterNetEvent('indigo-phone:server:documents_Save_Note_As', function(data, Receiver)
    local src = source
    ...
    elseif data.Type == "PermSend" then
        ...
        if Note[1] then
            ...
            TriggerClientEvent('indigo-phone:client:CustomNotification', tonumber(data.StateID), 'DOCUMENTS', 'New Document', 'fas fa-folder', '#d9d9d9', 5000)
        end
    end

    local Notes = exports.oxmysql:executeSync('SELECT * FROM phone_note WHERE citizenid = ?', {CID})
    Wait(100)
    TriggerClientEvent('indigo-phone:RefReshNotes_Free_Documents', src, Notes)
end)
