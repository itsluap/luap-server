            ...
            for _, room in pairs(chatrooms) do
                ...
                if Player then
                    if Player.PlayerData.money.bank >= price then
                        ...
                        TriggerEvent('indigo-phone:server:sendNewEventMail', room.room_owner_id, {
                            room.room_owner_id,
...
QBCore.Functions.CreateCallback('indigo-phone:server:GetGroupChatMessages', function(_, cb, roomID)
...
QBCore.Functions.CreateCallback('indigo-phone:server:SearchGroupChatMessages', function(source, cb, roomID, searchTerm)
    local src = source
    ...
    if isOwnerOfRoom(Player.PlayerData.citizenid, roomID) or isMemberOfRoom(Player.PlayerData.citizenid, roomID) then
        ...
        TriggerClientEvent('indigo-phone:client:notification', src, 'Discord', 'You must be a member or room owner to search.')
        cb(false)
    end
end)

QBCore.Functions.CreateCallback('indigo-phone:server:GetPinnedMessages', function(source, cb, roomID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if isOwnerOfRoom(Player.PlayerData.citizenid, roomID) or isMemberOfRoom(Player.PlayerData.citizenid, roomID) then
        ...
        TriggerClientEvent('indigo-phone:client:notification', src, 'Discord', 'You must be a member or room owner to fetch that.')
        cb(false)
    end
end)


QBCore.Functions.CreateCallback('indigo-phone:server:TryPinCode', function(_, cb, pinCode, roomID)
...
QBCore.Functions.CreateCallback('indigo-phone:server:IsRoomOwner', function(source, cb, roomID)
...
RegisterNetEvent('indigo-phone:server:SendGroupChatMessage', function(messageData, systemMessage, roomID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if messageData and not systemMessage then
        if isOwnerOfRoom(Player.PlayerData.citizenid, roomID) or isMemberOfRoom(Player.PlayerData.citizenid, roomID) then
            ...
            TriggerClientEvent('indigo-phone:client:RefreshGroupChat', -1, src, messageData)
            ...
            TriggerClientEvent('indigo-phone:client:notification', src, 'Discord', 'You must be a member or room owner to send messages.')
        end
    else
        ...
        TriggerClientEvent('indigo-phone:client:RefreshGroupChat', -1, src, systemMessage)
    end
end)

QBCore.Functions.CreateCallback('indigo-phone:server:JoinGroupChat', function(source, cb, updatedRooms, roomID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not isMemberOfRoom(Player.PlayerData.citizenid, roomID) then
        ...
        TriggerClientEvent('indigo-phone:client:RefreshChatRooms', -1, ChatRooms)
        cb(true)
    end
end)

RegisterNetEvent('indigo-phone:server:LeaveGroupChat', function(updatedRooms, roomID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local members = {}
...
RegisterNetEvent('indigo-phone:server:ChangeRoomPin', function(updatedRooms, roomID, pin)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('indigo-phone:server:DeactivateRoom', function(updatedRooms, roomID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('indigo-phone:server:PurchaseRoom', function(source, cb, price, roomData)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.money.bank >= price then
        ...
        if Config.RenewedBanking then
            exports['Renwed-Banking']:handleTransaction(cid, "Discord App", price, "Discord Channel Purchase", roomData.room_owner_name, "Discord", "withdraw")
        end
...
RegisterNetEvent('indigo-phone:server:ToggleMessagePin', function(messageID, roomID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
