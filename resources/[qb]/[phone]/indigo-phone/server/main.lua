...
QBCore.Functions.CreateCallback('indigo-phone:server:GetCallState', function(source, cb, ContactData)
...
QBCore.Functions.CreateCallback('indigo-phone:server:GetPhoneData', function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback('indigo-phone:server:FetchResult', function(_, cb, input)
...
QBCore.Functions.CreateCallback("indigo-phone:server:GetWebhook",function(_, cb)
	cb(WebHook)
end)

-- Events
RegisterNetEvent('indigo-phone:server:SetCallState', function(bool)
    local src = source
...
RegisterNetEvent('indigo-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    local Target = QBCore.Functions.GetPlayerByPhone(tostring(TargetData.number))
    if not Target or not Ply then return end

    TriggerClientEvent('indigo-phone:client:GetCalled', Target.PlayerData.source, Ply.PlayerData.charinfo.phone, CallId, AnonymousCall)
end)

RegisterNetEvent('indigo-phone:server:EditContact', function(newName, newNumber, oldName, oldNumber)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('indigo-phone:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('indigo-phone:server:AddNewContact', function(name, number)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('indigo-phone:server:AddRecentCall', function(type, data)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('indigo-phone:client:AddRecentCall', src, data, label, type)

    local Target = QBCore.Functions.GetPlayerByPhone(data.number)
    if not Target then return end

    TriggerClientEvent('indigo-phone:client:AddRecentCall', Target.PlayerData.source, {
...
RegisterNetEvent('indigo-phone:server:GiveContactDetails', function(PlayerId)
    ...
    TriggerClientEvent('indigo-phone:client:giveContactRequest', PlayerId, contactInfo)
end)

RegisterNetEvent('indigo-phone:server:acceptContactRequest', function(contactInfo)
    ...
    TriggerClientEvent('indigo-phone:client:updateContactInfo', src, contactInfo)
end)

RegisterNetEvent('indigo-phone:server:CancelCall', function(ContactData)
    local Ply = QBCore.Functions.GetPlayerByPhone(tostring(ContactData.TargetData.number))
    if not Ply then return end
    TriggerClientEvent('indigo-phone:client:CancelCall', Ply.PlayerData.source)
end)

RegisterNetEvent('indigo-phone:server:AnswerCall', function(CallData)
    local Ply = QBCore.Functions.GetPlayerByPhone(CallData.TargetData.number)
    if not Ply then return end

    TriggerClientEvent('indigo-phone:client:AnswerCall', Ply.PlayerData.source)
end)

RegisterNetEvent('indigo-phone:server:SaveMetaData', function(MData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
