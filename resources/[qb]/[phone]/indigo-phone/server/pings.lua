...
RegisterNetEvent("indigo-phone:server:sendPing", function(id)
    local src = source
    ...
    if Player.PlayerData.citizenid ~= Other.PlayerData.citizenid then
        TriggerClientEvent("indigo-phone:client:sendNotificationPing", Shitter, info)
...
RegisterNetEvent("indigo-phone:server:sendingPing", function(Other, Player, Name, OtherName)
    TriggerClientEvent('indigo-phone:client:CustomNotification', Player, "PING", OtherName..' Accepted Your Ping!', 'fas fa-map-pin', '#b3e0f2', 7500)
    TriggerClientEvent("indigo-phone:client:sendPing", Other, Name, GetEntityCoords(GetPlayerPed(Player)))
end)