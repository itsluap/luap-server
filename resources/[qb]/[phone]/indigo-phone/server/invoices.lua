...
RegisterNetEvent('indigo-phone:server:InvoiceHandler')

-- EVENT HANDLER(S) --

-- Has player paid something this --
--[[AddEventHandler('indigo-phone:server:InvoiceHandler', function(paid, amount, source, resource)
    ...
    if paid and resource == GetCurrentResourceName() then
        if amount >= config.minPayment then
            if Config.RenewedBanking then
                ...
                exports['Renwed-Banking']:handleTransaction(cid, "Phone Invoice", amount, "Paid off phone invoice of $"..amount, name, name, "withdraw")
            end
            -- Do shit
        end
    end
...
RegisterNetEvent('indigo-phone:server:PayMyInvoice', function(society, amount, invoiceId, sendercitizenid, resource)
    local src = source
    ...
    if Player.PlayerData.money.bank >= amount then
        ...
        if SenderPly then
            TriggerClientEvent('indigo-phone:client:CustomNotification', SenderPly.PlayerData.source,
        ...
        TriggerClientEvent('indigo-phone:client:RemoveInvoiceFromTable', src, invoiceId)
        TriggerEvent("indigo-phone:server:InvoiceHandler", true, amount, src, resource)

        exports.oxmysql:execute('DELETE FROM phone_invoices WHERE id = ?', {invoiceId})
    end
end)

RegisterNetEvent('indigo-phone:server:DeclineMyInvoice', function(amount, invoiceId, sendercitizenid, resource)
    ...
    TriggerClientEvent('indigo-phone:client:RemoveInvoiceFromTable', source, invoiceId)
    TriggerEvent("indigo-phone:server:InvoiceHandler", false, amount, source, resource)
end)


RegisterNetEvent('indigo-phone:server:CreateInvoice', function(billed, biller, amount)
    ...
    MySQL.Async.insert('INSERT INTO phone_invoices (citizenid, amount, society, sender, sendercitizenid) VALUES (?, ?, ?, ?, ?)',{
        ...
        if id then
            TriggerClientEvent('indigo-phone:client:AcceptorDenyInvoice', billedCID.PlayerData.source, id, billerInfo.PlayerData.charinfo.firstname, billerInfo.PlayerData.job.name, billerInfo.PlayerData.citizenid, cash, resource)
        end
    end)
end)

QBCore.Functions.CreateCallback('indigo-phone:server:GetInvoices', function(source, cb)
