...
RegisterNetEvent('indigo-phone:server:AddAdvert', function(msg, url)
    local src = source
    ...
    TriggerClientEvent('indigo-phone:client:UpdateAdverts', -1, Adverts, name, src)
end)

RegisterNetEvent('indigo-phone:server:DeleteAdvert', function()
    local k = GetAdvertFromNumb(source)
    if not k then return end
    table.remove(Adverts, k)
    TriggerClientEvent('indigo-phone:client:UpdateAdverts', -1, Adverts)
end)

RegisterNetEvent('indigo-phone:server:flagAdvert', function(number)
    local src = source
