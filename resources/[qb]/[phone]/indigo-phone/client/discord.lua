...
RegisterNetEvent('indigo-phone:client:notification', function(app, message)
    SendNUIMessage({
...
RegisterNetEvent('indigo-phone:client:RefreshChatRooms', function(ChatRooms)
    PhoneData.ChatRooms = ChatRooms
    SendNUIMessage({action = "RefreshChatRooms", Rooms = PhoneData.ChatRooms})
end)

RegisterNetEvent('indigo-phone:client:RefreshGroupChat', function(src, message)
    ...
    for _, room in pairs(PhoneData.ChatRooms) do
        if room.id == data.roomID then
            ...
            if not room.room_pin then
                QBCore.Functions.TriggerCallback('indigo-phone:server:GetGroupChatMessages', function(messages)
                    cb(messages)
                end, data.roomID)
            else
                if next(memberList) then
    ...
    for _, room in pairs(PhoneData.ChatRooms) do
        if(room.id == Room) then
            ...
            if next(memberList) then
                for _, memberData in pairs(memberList) do
                    if Player == memberData.cid or Player == room.room_owner_id then
                        QBCore.Functions.TriggerCallback('indigo-phone:server:SearchGroupChatMessages', function(messages)
                            cb(messages)
                        end, Room, SearchTerm)
                        break
                    end
    ...
    for _, room in pairs(PhoneData.ChatRooms) do
        if(room.id == Room) then
            ...
            if next(memberList) then
                ...
                for _, memberData in pairs(memberList) do
                    if Player == memberData.cid or Player == room.room_owner_id then
                        QBCore.Functions.TriggerCallback('indigo-phone:server:GetPinnedMessages', function(messages)
                            cb(messages)
                        end, Room)
                    end
                    break
    ...
    for _, room in pairs(PhoneData.ChatRooms) do
        if(room.id == Message.room_id) then
            ...
            if next(memberList) then
                for _, memberData in pairs(memberList) do
                    if Player == memberData.cid or Player == room.room_owner_id then
                        TriggerServerEvent("indigo-phone:server:SendGroupChatMessage", Message, nil, data.roomID)
...
RegisterNUICallback('JoinGroupChat', function(data, cb)
    ...
    if not room then
        ...
        if isMemberOfRoom(Player, roomID) then
            ...
            if roomPin then
                QBCore.Functions.TriggerCallback('indigo-phone:server:TryPinCode', function(result)
                    if result then
                        ...
                        if next(members) then
                            ...
                            QBCore.Functions.TriggerCallback("indigo-phone:server:JoinGroupChat",function(success)
                                if success then
                                    TriggerServerEvent("indigo-phone:server:SendGroupChatMessage", nil, {
                                        room_id = roomID,
                                        messageType = "SYSTEM",
...
RegisterNUICallback('LeaveGroupChat', function(data, cb)
    ...
    if not room then
        ...
        if isMemberOfRoom(Player, roomID) then
            ...
            TriggerServerEvent('indigo-phone:server:LeaveGroupChat', PhoneData.ChatRooms, roomID)
            cb(true)
        else
            cb(false)
        end
...
RegisterNUICallback('ChangeRoomPin', function(data, cb)
    ...
    if not room then
        ...
        if pin then
            QBCore.Functions.TriggerCallback('indigo-phone:server:IsRoomOwner', function(isOwner)
                if isOwner then
                    for k, v in pairs(PhoneData.ChatRooms) do
                        if(v.id == roomID) then
                            if pin == '' then
                    ...
                    TriggerServerEvent('indigo-phone:server:ChangeRoomPin', PhoneData.ChatRooms, roomID, pin)
                    cb(true)
                else
                    cb(false)
                end
        ...
        if room.id == data.roomID then
            QBCore.Functions.TriggerCallback('indigo-phone:server:IsRoomOwner', function(isOwner)
                if isOwner then
                    ...
                    TriggerServerEvent('indigo-phone:server:DeactivateRoom', PhoneData.ChatRooms, data.roomID)
                    cb(true)
                else
                    cb(false)
                end
...
RegisterNUICallback('ToggleMessagePin', function(data, cb)
    ...
    for _, room in pairs(PhoneData.ChatRooms) do
        if room.id == roomID then
            QBCore.Functions.TriggerCallback('indigo-phone:server:IsRoomOwner', function(isOwner)
                if isOwner then
                    TriggerServerEvent('indigo-phone:server:ToggleMessagePin', messageID, roomID)
                    cb(true)
                else
                    cb(false)
                end
...
RegisterNUICallback('CreateDiscordRoom', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback("indigo-phone:server:PurchaseRoom",function(status)
        cb(status)
    end, 250, roomData)
end)