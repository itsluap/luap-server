...
RegisterNUICallback('PostAdvert', function(data, cb)
    ...
    TriggerServerEvent('indigo-phone:server:AddAdvert', data.message, url)
    cb("ok")
end)


RegisterNUICallback('FlagAdvert', function(data, cb)
    TriggerServerEvent('indigo-phone:server:flagAdvert', data.number)
    cb("ok")
end)

RegisterNUICallback("DeleteAdvert", function(_, cb)
    TriggerServerEvent("indigo-phone:server:DeleteAdvert")
    cb("ok")
end)
...
RegisterNetEvent('indigo-phone:client:UpdateAdverts', function(Adverts, LastAd, src)
