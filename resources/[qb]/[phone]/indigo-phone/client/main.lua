...
exports["qb-target"]:AddTargetModel(PublicPhoneobject, {
    options = {
        {
            type = "client",
            event = "indigo-phone:client:publocphoneopen",
...
local function LoadPhone()
    QBCore.Functions.TriggerCallback('indigo-phone:server:GetPhoneData', function(pData)
...
local function CancelCall()
    TriggerServerEvent('indigo-phone:server:CancelCall', PhoneData.CallData)
    ...
    TriggerServerEvent('indigo-phone:server:SetCallState', false)
...
local function CallContact(CallData, AnonymousCall)
    ...
    TriggerServerEvent('indigo-phone:server:CallContact', PhoneData.CallData.TargetData, PhoneData.CallData.CallId, AnonymousCall)
    TriggerServerEvent('indigo-phone:server:SetCallState', true)
...
local function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        TriggerServerEvent('indigo-phone:server:AnswerCall', PhoneData.CallData)
...
RegisterNUICallback('SetBackground', function(data, cb)
    ...
    TriggerServerEvent('indigo-phone:server:SaveMetaData', PhoneData.MetaData)
    cb('ok')
end)
...
RegisterNUICallback('AddNewContact', function(data, cb)
    ...
    TriggerServerEvent('indigo-phone:server:AddNewContact', data.ContactName, data.ContactNumber)
end)
...
RegisterNUICallback('EditContact', function(data, cb)
    ...
    Wait(100)
    cb(PhoneData.Contacts)
    TriggerServerEvent('indigo-phone:server:EditContact', NewName, NewNumber, OldName, OldNumber)
end)
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    QBCore.Functions.TriggerCallback('indigo-phone:server:FetchResult', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('DeleteContact', function(data, cb)
    ...
    TriggerServerEvent('indigo-phone:server:RemoveContact', Name, Number)
end)
...
RegisterNUICallback('CallContact', function(data, cb)
    QBCore.Functions.TriggerCallback('indigo-phone:server:GetCallState', function(CanCall, IsOnline)
        local status = {
            CanCall = CanCall,
            IsOnline = IsOnline,
...
RegisterNUICallback("TakePhoto", function(_, cb)
    ...
    while true do
        ...
        elseif IsControlJustPressed(1, 176) then
            QBCore.Functions.TriggerCallback("indigo-phone:server:GetWebhook",function(hook)
                ...
                exports['screenshot-basic']:requestScreenshotUpload(tostring(hook), "files[]", function(uploadData)
                    ...
                    DestroyMobilePhone()
                    CellCamActivate(false, false)
                    TriggerServerEvent('indigo-phone:server:addImageToGallery', image.attachments[1].proxy_url)
                    Wait(400)
...
RegisterNetEvent('indigo-phone:client:AddRecentCall', function(data, time, type)
...
RegisterNetEvent('indigo-phone:client:CancelCall', function()
    if PhoneData.CallData.CallType == "ongoing" then
        SendNUIMessage({
...
RegisterNetEvent('indigo-phone:client:GetCalled', function(CallerNumber, CallId, AnonymousCall)
    local RepeatCount = 0
    local CallData = {
        number = CallerNumber,
            ...
            if not PhoneData.CallData.AnsweredCall then
                if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                    if PhoneData.CallData.InCall then
                        ...
                        TriggerServerEvent('indigo-phone:server:AddRecentCall', "missed", CallData)
                        break
                    end
                    Wait(Config.RepeatTimeout)
                else
...
RegisterNetEvent('indigo-phone:client:AnswerCall', function()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        PhoneData.CallData.CallType = "ongoing"
        PhoneData.CallData.AnsweredCall = true
        PhoneData.CallData.CallTime = 0
...
RegisterNetEvent('indigo-phone:client:clearAppAlerts', function()
    Config.PhoneApplications["phone"].Alerts = 0
    SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
end)
...
RegisterNetEvent('indigo-phone:client:publocphoneopen',function()
    SetNuiFocus(true, true)
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
        QBCore.Functions.TriggerCallback('indigo-phone:server:CanTransferMoney', function(Transferd)
            if Transferd then
...
RegisterNetEvent('indigo-phone:client:TransferMoney', function(amount, newmoney)
...
RegisterNetEvent("indigo-phone-new:client:BankNotify", function(text)
    SendNUIMessage({
...
RegisterNetEvent('indigo-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
...
RegisterNetEvent('indigo-phone:client:GiveContactDetails', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local PlayerId = GetPlayerServerId(player)
        TriggerServerEvent('indigo-phone:server:GiveContactDetails', PlayerId)
    else
        QBCore.Functions.Notify("No one nearby!", "error")
    end
end)

RegisterNetEvent("indigo-phone:client:giveContactRequest", function(contactInfo)
    local success = exports['indigo-phone']:PhoneNotification("CONTACT REQUEST", contactInfo.name..' contact request', 'fas fa-phone', '#b3e0f2', "NONE", 'fas fa-check-circle', 'fas fa-times-circle')
    if success then
        TriggerServerEvent('indigo-phone:server:acceptContactRequest', contactInfo)
    end
end)

RegisterNetEvent('indigo-phone:client:updateContactInfo', function(contactInfo)
    PhoneData.Contacts[#PhoneData.Contacts+1] = {
