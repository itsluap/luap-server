...
RegisterNUICallback('GetGroupsApp', function (_, cb)
    QBCore.Functions.TriggerCallback('indigo-phone:server:getAllGroups', function (getGroups)
        cb(getGroups)
    end)
end)

RegisterNetEvent('indigo-phone:client:RefreshGroupsApp', function(Groups, finish)
...
RegisterNetEvent('indigo-phone:client:AddGroupStage', function(_, stage)
    inJob = true
    SendNUIMessage({
...
RegisterNUICallback('jobcenter_CreateJobGroup', function(data, cb) --employment
    TriggerServerEvent('indigo-phone:server:jobcenter_CreateJobGroup', data)
    cb("ok")
end)

RegisterNUICallback('jobcenter_JoinTheGroup', function(data, cb) --employment
    TriggerServerEvent('indigo-phone:server:jobcenter_JoinTheGroup', data)
    cb("ok")
end)

RegisterNUICallback('jobcenter_leave_grouped', function(data, cb) --employment
    if not data then return end
    local success = exports['indigo-phone']:PhoneNotification("Job Center", 'Are you sure you want to leave the group?', 'fas fa-users', '#FFBF00', "NONE", 'fas fa-check-circle', 'fas fa-times-circle')
    if success then
        TriggerServerEvent('indigo-phone:server:jobcenter_leave_grouped', data)
    end
    cb("ok")
end)

RegisterNUICallback('jobcenter_DeleteGroup', function(data, cb) --employment
    TriggerServerEvent('indigo-phone:server:jobcenter_DeleteGroup', data)
    cb("ok")
end)


RegisterNUICallback('jobcenter_CheckPlayerNames', function(data, cb) --employment
    QBCore.Functions.TriggerCallback('indigo-phone:server:jobcenter_CheckPlayerNames', function(HasName)
        cb(HasName)
    end, data.id)
end)