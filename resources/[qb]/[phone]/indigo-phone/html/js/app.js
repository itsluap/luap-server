            ...
            if (QB.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "twitter") {
                    if (QB.Phone.Data.IsOpen) {
                        $.post('https://indigo-phone/GetTweets', JSON.stringify({}), function(Tweets){
                    ...
                    $.post('https://indigo-phone/GetBankContacts', JSON.stringify({}), function(contacts){
                        QB.Phone.Functions.LoadContactsWithNumber(contacts);
                    });
                    $.post('https://indigo-phone/GetInvoices', JSON.stringify({}), function(invoices){
                        QB.Phone.Functions.LoadBankInvoices(invoices);
                    });
                } else if (PressedApplication == "whatsapp") {
                    $.post('https://indigo-phone/GetWhatsappChats', JSON.stringify({}), function(chats){
                        QB.Phone.Functions.LoadWhatsappChats(chats);
                    });
                } else if (PressedApplication == "phone") {
                    $.post('https://indigo-phone/GetMissedCalls', JSON.stringify({}), function(recent){
                        QB.Phone.Functions.SetupRecentCalls(recent);
                    });
                    $.post('https://indigo-phone/ClearGeneralAlerts', JSON.stringify({
                        app: "phone"
                    }));
                } else if (PressedApplication == "mail") {
                    $.post('https://indigo-phone/GetMails', JSON.stringify({}), function(mails){
                    ...
                    $.post('https://indigo-phone/LoadAdverts', JSON.stringify({}), function(Adverts){
                        QB.Phone.Functions.RefreshAdverts(Adverts);
                    })
                } else if (PressedApplication == "garage") {
                    $.post('https://indigo-phone/SetupGarageVehicles', JSON.stringify({}), function(Vehicles){
                        SetupGarageVehicles(Vehicles);
                    })
                } else if (PressedApplication == "racing") {
                    $.post('https://indigo-phone/GetAvailableRaces', JSON.stringify({}), function(Races){
                        SetupRaces(Races);
                    });
                } else if (PressedApplication == "houses") {
                    $.post('https://indigo-phone/GetPlayerHouses', JSON.stringify({}), function(Houses){
                        SetupPlayerHouses(Houses);
                    });
                    $.post('https://indigo-phone/GetPlayerKeys', JSON.stringify({}), function(Keys){
                    ...
                    $.post('https://indigo-phone/GetAvailableTaxiDrivers', JSON.stringify({}), function(data){
                        SetupTaxiDrivers(data);
                    });
                }
                else if (PressedApplication == "gallery") {
                    $.post('https://indigo-phone/GetGalleryData', JSON.stringify({}), function(data){
                    ...
                    $.post('https://indigo-phone/GetJobs', JSON.stringify({}), function(data){
                        LoadEmploymentApp(data)
                    });
                } else if (PressedApplication == "debt") {
                    $.post('https://indigo-phone/GetPlayersDebt', JSON.stringify({}), function(data){
                        LoadDebtJob(data);
                    });
                }
                else if (PressedApplication == "gopro") {
                    $.post('https://indigo-phone/SetupGoPros', JSON.stringify({}), function(Cams){
                        SetupGoPros(Cams);
                    })
                }
                    ...
                    $.post('https://indigo-phone/GetChatRooms', JSON.stringify({}), function(ChatRooms){
...
$(document).on('click', '.mykeys-key', function(e){
    ...
    $.post('https://indigo-phone/SetHouseLocation', JSON.stringify({
        HouseData: KeyData
    }))
});
...
$(document).on('click', '.phone-take-camera-button', function(event){
    ...
    $.post('https://indigo-phone/TakePhoto', JSON.stringify({}),function(url){
        // setUpCameraApp(url)
    })
...
$(document).on('click', '.phone-silent-button', function(event){
    event.preventDefault();
    $.post('https://indigo-phone/phone-silent-button', JSON.stringify({}),function(Data){
        if(Data){
...
QB.Phone.Notifications.Custom.Add = function(icon, title, text, color, timeout, accept, deny) {
    $.post('https://indigo-phone/HasPhone', JSON.stringify({}), function(HasPhone){
        if (HasPhone) {
...
$(document).on('click', ".notification-accept", function() {
    $.post('https://indigo-phone/AcceptNotification', JSON.stringify({})),
    QB.Phone.Animations.TopSlideUp(".phone-notification-container-new", 150, -8);
    QB.Phone.Notifications.Timeout = null
...
$(document).on('click', ".notification-deny", function() {
    $.post('https://indigo-phone/DenyNotification', JSON.stringify({})),
    QB.Phone.Notifications.Timeout = null
