...
$('#leave-room').on('click', e => {
    ...
    $.post("https://indigo-phone/LeaveGroupChat", JSON.stringify({roomID: currentRoom, citizenid: cid}), function(status) {
        if(status) {
...
$('#join-room').on('click', e => {
    e.preventDefault()
    $.post("https://indigo-phone/JoinGroupChat", JSON.stringify({roomID: currentRoom}), function(status) {
        if(status) {
            $('.chat-room-join').hide()
...
$('#search-message').keypress(e => {
    if(e.which == 13) {
        ...
        if(keyword !== previousSearch && keyword !== "" && keyword.length >= 3) {
            ...
            $.post("https://indigo-phone/SearchGroupChatMessages", JSON.stringify({roomID: currentRoom, searchTerm: keyword}), function(messages) {
                if( ! messages) {
...
$('#pinned-messages').on('click', e => {
    ...
    if( ! pinnedOpen) {
        ...
        $.post("https://indigo-phone/GetPinnedMessages", JSON.stringify({roomID: currentRoom}), function(results) {
            if( ! results) {
...
function openChatRoom(id) {
    ...
    $.post("https://indigo-phone/GetGroupChatMessages", JSON.stringify({roomID: currentRoom}), function(messages) {
        RenderChatMessages(messages)  
    }) 
    $("#submit-message").prop('placeholder', 'Press [ENTER] to chat!')
...
$("#submit-message").on('keypress', (e) => {
    ...
    if(e.which === 13 && messageContent.length > 0) {
        ...
        $.post("https://indigo-phone/SendGroupChatMessage", JSON.stringify(messageData), function() {
            $("#submit-message").val("")
        })
    }
})
...
$("#send-message").on('click', () => {
    ...
    if(messageContent.length > 0) {
        ...
        $.post("https://indigo-phone/SendGroupChatMessage", JSON.stringify(messageData), function(status) {
            $("#submit-message").val("")
        })
    }
})
...
$(document).on('click', '.confirm', function(e) {
    ...
    if( ! isUserAnOwner(room)) {
        ...
        $.post("https://indigo-phone/DeactivateRoom", JSON.stringify({roomID: room}), function(status) {
            if(status) {
...
$(document).on('keypress', '#change-pin', function(e) {
    if(e.which == 13) {
        ...
        if(pinOk) {
            $.post("https://indigo-phone/ChangeRoomPin", JSON.stringify({pinCode: pin, roomID: id}), function(status) {
                if(status) {
                    $('#change-pin').val(pin)
                    $('#owned-rooms-list-close').trigger('click')
...
$("#create-room-confirm").on('click', function(e) {
    ...
    $.post("https://indigo-phone/CreateDiscordRoom", JSON.stringify({name: channelName, pass: channelPass}), function(status) {
        if(status) {
    ...
    if(e.which == 13) {
        ...
        if(chatroom) {
            if(chatroom.room_owner_id === QB.Phone.Data.PlayerData.citizenid) {
                ...
                let id = chatroom.id
                $("#join-room-code-close").trigger('click')
                $.post('https://indigo-phone/JoinGroupChat', JSON.stringify({roomID: id}), () => {
                    openChatRoom(chatroom.id)
...
$('#room-pin-input').keypress(e => {
    if(e.which == 13) {
        ...
        $.post('https://indigo-phone/JoinGroupChat', JSON.stringify({roomID: id, roomPin: pin}), (status) => {
            if(status) {
...
$(document).on('click', '.pin-message', function(e) {
    ...
    $.post("https://indigo-phone/ToggleMessagePin", JSON.stringify({roomID: room, messageID: message}), function(status) {
        if( ! status) {
...
$(document).on('click', '#remove-member', function(e){
    ...
    $.post("https://indigo-phone/LeaveGroupChat", JSON.stringify({roomID: room, citizenid: cid}), function(status) {
        if(status) {
            $(this).remove();
