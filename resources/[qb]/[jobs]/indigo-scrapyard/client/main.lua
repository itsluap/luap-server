...
RegisterNetEvent("QBCore:Client:OnPlayerLoaded", function()
    TriggerServerEvent("indigo-scrapyard:server:LoadVehicleList")
end)
        ...
        if not isBusy then
            if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
                if IsVehicleValid(GetEntityModel(vehicle)) then
                    ...
                    QBCore.Functions.TriggerCallback('indigo-scrapyard:checkOwnerVehicle',function(retval)
                        if retval then
                            isBusy = true
                            ...
                            QBCore.Functions.Progressbar("scrap_vehicle", Lang:t('text.demolish_vehicle'), scrapTime, false, true, {
                                ...
                                TriggerServerEvent("indigo-scrapyard:server:ScrapVehicle", GetVehicleKey(GetEntityModel(vehicle)))
