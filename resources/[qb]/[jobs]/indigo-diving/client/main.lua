...
local function callCops()
    ...
    if call == chance then
        TriggerServerEvent('indigo-diving:server:CallCops', coords)
    end
end
...
local function takeCoral(coral)
    ...
    QBCore.Functions.Progressbar("take_coral", Lang:t("info.collecting_coral"), times * 1000, false, true, {
        ...
        TriggerServerEvent('indigo-diving:server:TakeCoral', currentDivingLocation.area, coral, true)
        ClearPedTasks(ped)
...
local function sellCoral()
    ...
    QBCore.Functions.Progressbar("sell_coral_items", Lang:t("info.checking_pockets"), math.random(2000, 4000), false, true, {}, {}, {}, {}, function() -- Done
        ...
        TriggerServerEvent('indigo-diving:server:SellCoral')
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.TriggerCallback('indigo-diving:server:GetDivingConfig', function(config, area)
...
RegisterNetEvent('indigo-diving:client:NewLocations', function()
    QBCore.Functions.TriggerCallback('indigo-diving:server:GetDivingConfig', function(config, area)
        Config.CoralLocations = config
        setDivingLocation(area)
    end)
...
RegisterNetEvent('indigo-diving:client:UpdateCoral', function(area, coral, bool)
    Config.CoralLocations[area].coords.Coral[coral].PickedUp = bool
end)

RegisterNetEvent('indigo-diving:client:CallCops', function(coords, msg)
...
RegisterNetEvent("indigo-diving:client:setoxygenlevel", function()
    if oxgenlevell == 0 then
       oxgenlevell = 100 -- oxygenlevel
       QBCore.Functions.Notify(Lang:t("success.tube_filled"), 'success')
       TriggerServerEvent('indigo-diving:server:removeItemAfterFill')
    else
...
RegisterNetEvent('indigo-diving:client:UseGear', function()
    local ped = PlayerPedId()
