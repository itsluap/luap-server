...
RegisterNetEvent('indigo-diving:server:CallCops', function(coords)
    for _, Player in pairs(QBCore.Functions.GetQBPlayers()) do
        if Player then
            if Player.PlayerData.job.type == "leo" and Player.PlayerData.job.onduty then
                local msg = Lang:t("info.cop_msg")
                TriggerClientEvent('indigo-diving:client:CallCops', Player.PlayerData.source, coords, msg)
                local alertData = {
...
RegisterNetEvent('indigo-diving:server:SellCoral', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('indigo-diving:server:TakeCoral', function(area, coral, bool)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if (Config.CoralLocations[area].TotalCoral - 1) == 0 then
        ...
        TriggerClientEvent('indigo-diving:client:NewLocations', -1)
    else
    ...
    TriggerClientEvent('indigo-diving:client:UpdateCoral', -1, area, coral, bool)
end)

RegisterNetEvent('indigo-diving:server:removeItemAfterFill', function()
   local src = source
   local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('indigo-diving:server:GetDivingConfig', function(_, cb)
...
QBCore.Functions.CreateUseableItem("diving_gear", function(source)
    TriggerClientEvent("indigo-diving:client:UseGear", source)
end)

QBCore.Functions.CreateUseableItem("diving_fill", function(source)
    TriggerClientEvent("indigo-diving:client:setoxygenlevel", source)
end)
