...
CreateThread(function()
	for k, v in pairs(Config.Locations) do
		if v.garage then
			...
			Targets["VanGarage: "..k] =
				exports['indigo-target']:AddBoxZone("VanGarage: "..k, vector3(out.x, out.y, out.z-1.03), 0.8, 0.5, { name="VanGarage: "..k, heading = out[4]+180.0, debugPoly=Config.Debug, minZ=(out.z-1.03)-0.1, maxZ=(out.z-1.03)+1.3 },
					{ options = { { event = "jim-vanillaunicorn:client:Garage:Menu", icon = "fas fa-clipboard", label = Loc[Config.Lan].targetinfo["job_vehicles"], job = v.job, coords = v.garage.spawn, list = v.garage.list }, },
					distance = 2.0 })
