    ...
    CreateThread(function()
        while ControlListen do
            if IsControlJustReleased(0, 38) then
                TriggerEvent("indigo-garbagejob:client:MainMenu")
            end
            Wait(1)
        end
    end)
...
RegisterNetEvent('indigo-garbagejob:client:RequestRoute', function()
                ...
                for _,v in pairs(Config.Locations["vehicle"].coords) do
                    if not IsAnyVehicleNearPoint(vector3(v.x,v.y,v.z), 2.5) then
                        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
                            ...
                            TriggerServerEvent("indigo-garbagejob:server:payDeposit")
...
RegisterNetEvent('indigo-garbagejob:client:RequestPaycheck', function()
...
RegisterNetEvent('indigo-garbagejob:client:MainMenu', function()
    local MainMenu = {}
    MainMenu[#MainMenu+1] = {isMenuHeader = true,header = Lang:t("menu.header")}
    MainMenu[#MainMenu+1] = { header = Lang:t("menu.collect"),txt = Lang:t("menu.return_collect"),params = { event = 'indigo-garbagejob:client:RequestPaycheck',}}
    if not garbageVehicle or finished then
        MainMenu[#MainMenu+1] = { header = Lang:t("menu.route"), txt = Lang:t("menu.request_route"), params = { event = 'indigo-garbagejob:client:RequestRoute',}}
    end
