...
QBCore.Functions.CreateCallback('indigo-hotdogjob:server:HasMoney', function(source, cb)
...
QBCore.Functions.CreateCallback('indigo-hotdogjob:server:BringBack', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterNetEvent('indigo-hotdogjob:server:Sell', function(coords, amount, price)
    local src = source
    local pCoords = GetEntityCoords(GetPlayerPed(src))
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    if #(pCoords - coords) > 4 then exports['indigo-core']:ExploitBan(src, 'hotdog job') end
    Player.Functions.AddMoney('cash', tonumber(amount * price), 'hotdog')
end)

RegisterNetEvent('indigo-hotdogjob:server:UpdateReputation', function(quality)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if quality == "exotic" then
        if JobReputation["hotdog"] ~= nil and JobReputation["hotdog"] + 3 > Config.MaxReputation then
            ...
            TriggerClientEvent('indigo-hotdogjob:client:UpdateReputation', src, JobReputation)
            return
        end
...
QBCore.Commands.Add("removestand", Lang:t("info.command"), {}, false, function(source, _)
    TriggerClientEvent('indigo-hotdogjob:staff:DeletStand', source)
end, 'admin')
