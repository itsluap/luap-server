...
local function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        TriggerServerEvent('indigo-hotdogjob:server:UpdateReputation', Quality)
...
local function StartWorking()
    QBCore.Functions.TriggerCallback('indigo-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            if Config.UseTarget then
...
local function StopWorking()
    if DoesEntityExist(StandObject) then
        QBCore.Functions.TriggerCallback('indigo-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                DeleteObject(StandObject)
                ClearPedTasksImmediately(PlayerPedId())
                IsWorking = false
                            ...
                            options = {
                                {
                                    ...
                                    action = function(entity)
                                        ...
                                        TriggerServerEvent('indigo-hotdogjob:server:Sell', pedCoords, HotdogsForSale, SellingPrice)
                                        SellingData.HasTarget = false
                                        LoadAnim('mp_common')
...
RegisterNetEvent('indigo-hotdogjob:client:UpdateReputation', function(JobRep)
    PlayerData.metadata["jobrep"] = JobRep
    UpdateLevel()
end)

RegisterNetEvent('indigo-hotdogjob:client:ToggleSell', function()
...
RegisterNetEvent('indigo-hotdogjob:staff:DeletStand', function()
    local ped = PlayerPedId()
