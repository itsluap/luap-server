...
RegisterServerEvent('jim-payments:Tickets:Give', function(data, biller, gang)
	...
	if biller then -- If this is found, it ISN'T a phone payment, so add money to society here
		if gang then
			if Config.RenewedBanking then exports['Renwed-Banking']:addAccountMoney(tostring(biller.PlayerData.gang.name), data.amount - takecomm)
				if Config.Debug then print("^5Debug^7: ^3Renwed-Banking^7(^3Gang^7): ^2Adding ^7$^6"..data.amount - takecomm.." ^2to account ^7'^6"..tostring(biller.PlayerData.gang.name).."^7' ($^6"..exports['Renwed-Banking']:getAccountMoney(biller.PlayerData.gang.name).."^7)") end
			else
		...
		elseif not gang then
			if Config.RenewedBanking then exports['Renwed-Banking']:addAccountMoney(tostring(biller.PlayerData.job.name), data.amount - takecomm)
				if Config.Debug then print("^5Debug^7: ^3Renwed-Banking^7(^3Job^7): ^2Adding ^7$^6"..data.amount - takecomm.." ^2to account ^7'^6"..tostring(biller.PlayerData.job.name).."^7' ($^6"..tostring(exports['Renwed-Banking']:getAccountMoney(biller.PlayerData.job.name)).."^7)") end
			else
...
RegisterServerEvent("jim-payments:server:PolCharge", function(citizen, price)
	...
	if price > 0 then
		if not Config.FineJobConfirmation then
			...
			if Config.RenewedBanking then exports['Renwed-Banking']:addAccountMoney(tostring(biller.PlayerData.job.name), (price - commission))
				if Config.Debug then print("^5Debug^7: ^3Renwed-Banking^7(^3Job^7): ^2Adding ^7$^6"..(price - commission).." ^2to account ^7'^6"..tostring(biller.PlayerData.job.name).."^7' ($^6"..exports['Renwed-Banking']:getAccountMoney((biller.PlayerData.job.name).."^7)")) end
			else
...
RegisterServerEvent("jim-payments:server:PolPopup", function(data)
	...
	if data.accept == true then
		...
		if Config.RenewedBanking then exports['Renwed-Banking']:addAccountMoney(tostring(biller.PlayerData.job.name), data.amount - commission)
			if Config.Debug then print("^5Debug^7: ^3Renwed-Banking^7(^3Job^7): ^2Adding ^7$^6"..(data.amount - commission).." ^2to account ^7'^6"..tostring(biller.PlayerData.job.name).."^7' ($^6"..exports['Renwed-Banking']:getAccountMoney((biller.PlayerData.job.name).."^7)")) end
		else
