...
RegisterNetEvent('indigo-cityhall:client:getIds', function()
    TriggerServerEvent('indigo-cityhall:server:getIDs')
end)

RegisterNetEvent('indigo-cityhall:client:sendDriverEmail', function(charinfo)
...
RegisterNUICallback('requestId', function(id, cb)
    ...
    if inRangeCityhall and license and id.cost == license.cost then
        TriggerServerEvent('indigo-cityhall:server:requestId', id.type, closestCityhall)
...
RegisterNUICallback('applyJob', function(job, cb)
    if inRangeCityhall then
        TriggerServerEvent('indigo-cityhall:server:ApplyJob', job, Config.Cityhalls[closestCityhall].coords)
    else
        QBCore.Functions.Notify(Lang:t('error.not_in_range'), 'error')
    end
    cb('ok')
...
CreateThread(function()
    initBlips()
    spawnPeds()
    QBCore.Functions.TriggerCallback('indigo-cityhall:server:receiveJobs', function(result)
        SendNUIMessage({
            action = 'setJobs',
            jobs = result
        })
