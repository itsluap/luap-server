...
Close = function() {
    ...
    $.post('https://indigo-cityhall/close');
...
$('.cityhall-option-block').click(function(e){
    ...
    if (blockPage == "identity") {
        ...
        $.post('https://indigo-cityhall/requestLicenses', JSON.stringify({}), function(licenses){
...
$(".request-identity-button").click(function(e){
    e.preventDefault();
    $.post('https://indigo-cityhall/requestId', JSON.stringify({
...
$(document).on('click', '.apply-job-button', function(e){
    e.preventDefault();
    $.post('https://indigo-cityhall/applyJob', JSON.stringify(selectedJobId))
    ShowCheckmark();
    ResetPages();
});
