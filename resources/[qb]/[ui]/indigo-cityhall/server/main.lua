...
QBCore.Functions.CreateCallback('indigo-cityhall:server:receiveJobs', function(_, cb)
    cb(availableJobs)
end)

-- Events

RegisterNetEvent('indigo-cityhall:server:requestId', function(item, hall)
    local src = source
...
RegisterNetEvent('indigo-cityhall:server:sendDriverTest', function(instructors)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    for i = 1, #instructors do
        ...
        if SchoolPlayer then
            TriggerClientEvent("indigo-cityhall:client:sendDriverEmail", SchoolPlayer.PlayerData.source, Player.PlayerData.charinfo)
        else
            local mailData = {
                sender = "Township",
            ...
            TriggerEvent("indigo-phone:server:sendNewMailToOffline", citizenid, mailData)
        end
    end
