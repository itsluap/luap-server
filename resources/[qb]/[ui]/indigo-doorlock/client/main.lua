...
local function isAuthorized(door)
	...
	if door.items then
		local p = promise.new()
		QBCore.Functions.TriggerCallback('indigo-doorlock:server:checkItems', function(result)
			p:resolve(result)
...
function SetupDoors()
	local p = promise.new()
	QBCore.Functions.TriggerCallback('indigo-doorlock:server:setupDoors', function(result)
		p:resolve(result)
	end)
...
RegisterNetEvent('indigo-doorlock:client:setState', function(serverId, doorID, state, src, enableAnimation, enableSounds)
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
	...
	--TriggerEvent('indigo-lockpick:client:openLockpick', lockpickFinish)
	exports['ps-ui']:Circle(function(success)
		if success then
			...
			Wait(1800)
			TriggerServerEvent('indigo-doorlock:server:updateState', closestDoor.id, false, false, true, false) -- Broadcast new state of the door to everyone
		else
			QBCore.Functions.Notify(Lang:t("error.lockpick_fail"), 'error', 2500)
			if math.random(1,100) <= 17 then
				if usingAdvanced then
					TriggerServerEvent("indigo-doorlock:server:removeLockpick", "advancedlockpick")
					TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items["advancedlockpick"], "remove")
				else
					TriggerServerEvent("indigo-doorlock:server:removeLockpick", "lockpick")
...
RegisterNetEvent('indigo-doorlock:client:addNewDoor', function()
	...
	if doorData.doortype == 'door' or doorData.doortype == 'sliding' or doorData.doortype == 'garage' then
		...
		TriggerServerEvent('indigo-doorlock:server:saveNewDoor', doorData, false)
		canContinue = true
	else
		...
		doorData.model = model
		doorData.heading = heading
		TriggerServerEvent('indigo-doorlock:server:saveNewDoor', doorData, true)
		canContinue = true
	end
end)

RegisterNetEvent('indigo-doorlock:client:newDoorAdded', function(data, id, creatorSrc)
	Config.DoorList[id] = data
	TriggerEvent('indigo-doorlock:client:setState', creatorSrc, id, data.locked, false, true, true)
end)

RegisterNetEvent('indigo-doorlock:client:ToggleDoorDebug', function()
...
RegisterCommand('toggledoorlock', function()
	...
	TriggerServerEvent('indigo-doorlock:server:updateState', closestDoor.id, locked, src, false, false, true, true) -- Broadcast new state of the door to everyone
end, false)
...
RegisterCommand('remotetriggerdoor', function()
	...
	TriggerServerEvent('indigo-doorlock:server:updateState', nearestDoor.id, not nearestDoor.data.locked, NetworkGetNetworkIdFromEntity(playerPed), false, false, true, true) -- Broadcast new state of the door to everyone
end, false)
