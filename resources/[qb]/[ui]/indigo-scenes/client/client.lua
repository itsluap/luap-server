...
RegisterNetEvent('indigo-scenes:createscene', function()
    OpenMenu()
    TriggerServerEvent("InteractSound_SV:PlayOnSource", "monkeyopening", 0.05)
end)
...
RegisterNetEvent('indigo-scenes:deletescene', function()
    ToggleDeletionLaser()
end)
...
RegisterNetEvent('indigo-scenes:client:UpdateAllScenes', function(list)
    scenes = list
end)
...
function GetScenes()
    QBCore.Functions.TriggerCallback('indigo-scenes:server:GetScenes', function(list)
        scenes = list
    end)
end
        ...
        CreateThread(function()
            while creationLaser do
                ...
                if IsControlJustReleased(0, 38) then
                    creationLaser = false
                    if hit then
                        TriggerServerEvent('indigo-scenes:server:CreateScene', data)
                    else
...
function DeleteScene(coords)
    ...
    if closestScene then
        ...
        TriggerServerEvent('indigo-scenes:server:DeleteScene', closestScene)
    else
