...
Citizen.CreateThread(function()
    ...
    -- Sync Image
    QBCore.Functions.TriggerCallback('indigo-whiteboard:getCurrentImage', function(images)
    ...
    for k,v in pairs(Config.Locations) do
        ...
        exports['qb-target']:AddBoxZone(k..'_Target', v.Target.coords, v.Target.length, v.Target.width, {
            ...
            options = {
                {
                    type = "client",
                    event = "indigo-whiteboard:changewhiteboardurl",
                    location = k,
                    icon = "fa fa-camera",
                    label = "Change Image",
                    job = v.job
...
AddEventHandler("indigo-whiteboard:changewhiteboardurl", function(data)
    if data.url then
        ...
        if link then
            TriggerServerEvent("indigo-whiteboard:changewhiteboard", link, data.location)
        end
    end
end)

RegisterNetEvent("indigo-whiteboard:changewhiteboardcli", function(pUrl, pRoom)
