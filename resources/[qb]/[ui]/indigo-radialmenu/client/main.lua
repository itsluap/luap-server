    ...
    if Vehicle ~= 0 then
        ...
        if not IsVehicleOnAllWheels(Vehicle) then
            VehicleMenu.items[#VehicleMenu.items+1] = {
                ...
                icon = 'car-burst',
                type = 'client',
                event = 'indigo-radialmenu:flipVehicle',
                shouldClose = true
            }
        end
    end
...
RegisterNetEvent('indigo-radialmenu:client:noPlayers', function()
    QBCore.Functions.Notify(Lang:t("error.no_people_nearby"), 'error', 2500)
end)

RegisterNetEvent('indigo-radialmenu:client:openDoor', function(data)
        ...
        if closestVehicle ~= GetVehiclePedIsIn(ped) then
            ...
            if GetVehicleDoorAngleRatio(closestVehicle, door) > 0.0 then
                if not IsVehicleSeatFree(closestVehicle, -1) then
                    TriggerServerEvent('indigo-radialmenu:trunk:server:Door', false, plate, door)
                else
                    SetVehicleDoorShut(closestVehicle, door, false)
                end
            else
                if not IsVehicleSeatFree(closestVehicle, -1) then
                    TriggerServerEvent('indigo-radialmenu:trunk:server:Door', true, plate, door)
                else
...
RegisterNetEvent('indigo-radialmenu:client:setExtra', function(data)
    local string = data.id
...
RegisterNetEvent('indigo-radialmenu:trunk:client:Door', function(plate, door, open)
...
RegisterNetEvent('indigo-radialmenu:client:ChangeSeat', function(data)
    local Veh = GetVehiclePedIsIn(PlayerPedId())
