...
RegisterServerEvent("indigo-clothing:saveSkin")
AddEventHandler('indigo-clothing:saveSkin', function(model, skin)
    local src = source
...
AddEventHandler("qb-clothes:saveOutfit", function(outfitName, model, skinData)
    ...
    if model ~= nil and skinData ~= nil then
        ...
        MySQL.insert('INSERT INTO player_outfits (citizenid, outfitname, model, skin, outfitId) VALUES (?, ?, ?, ?, ?)', {
            ...
            if result[1] ~= nil then
                TriggerClientEvent('indigo-clothing:client:reloadOutfits', src, result)
            else
                TriggerClientEvent('indigo-clothing:client:reloadOutfits', src, nil)
            end
        end)
    end
end)

RegisterServerEvent("indigo-clothing:server:removeOutfit")
AddEventHandler("indigo-clothing:server:removeOutfit", function(outfitName, outfitId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('indigo-clothing:server:getOutfits', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local anusVal = {}
