$(document).on('click', '.accept-button', function(e) {
    ...
    $.post('http://dream-stripclub/Click', JSON.stringify({}))
    if (PriceAmount != '' && PriceAmount != undefined && NoteAmount != '' && NoteAmount != undefined) {
        $.post('http://dream-stripclub/AddPrice', JSON.stringify({Price: PriceAmount, Note: NoteAmount}))
        $.post('http://dream-stripclub/CloseNui', JSON.stringify({}))
        ...
        $.post('http://dream-stripclub/ErrorClick', JSON.stringify({}))
    }
});
...
$(document).on('click', '.payment', function(e) {
    ...
    if (NumberId != null && Note!= null && Price != null) {
        $.post('http://dream-stripclub/PayReceipt', JSON.stringify({Price: Price, Note: Note, Id: NumberId}))
        $.post('http://dream-stripclub/CloseNui', JSON.stringify({}))
        $(".main-container").animate({"top": "-30vh"}, 250, function() {
            $('.items').html('');
            $('.main-container').css("display", "none");
...
SetupPaymentsStrip = function(data) {
    for (const [key, value] of Object.entries(data)) {
        if (value != undefined && value != null) {
            var CurrentId = key
            var AddOption = '<div class="payment" data-price='+value['Price']+' data-note="'+value['Note']+'" data-id='+CurrentId+'><p>Cost: $'+value['Price']+' <br> Note: '+value['Note']+'</p></div>'
