...
AddEventHandler('qb-burgershot:server:give:payment', function(PlayerId)
    ...
    if Player ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', source, 'This is not right..', 'error')
    end
end)

QBCore.Commands.Add("setburger", "Hire a burgershot employee", {{name="id", help="Player ID"}}, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    local TargetPlayer = QBCore.Functions.GetPlayer(tonumber(args[1]))
    if Player.PlayerData.metadata['ishighcommand'] and Player.PlayerData.job.name == 'burger' then
        if TargetPlayer ~= nil then
            TriggerClientEvent('QBCore:Notify', TargetPlayer.PlayerData.source, 'You have been hired as a burgershot employee!', 'success')
            TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, 'You hired '..TargetPlayer.PlayerData.charinfo.firstname..' '..TargetPlayer.PlayerData.charinfo.lastname..' as a burgershot employee!', 'success')
            TargetPlayer.Functions.SetJob('burger')
        end
    end
end)

QBCore.Commands.Add("fireburger", "Fire a burgershot employee", {{name="id", help="Player ID"}}, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    local TargetPlayer = QBCore.Functions.GetPlayer(tonumber(args[1]))
    if Player.PlayerData.metadata['ishighcommand'] and Player.PlayerData.job.name == 'burger' then
        if TargetPlayer ~= nil and TargetPlayer.PlayerData.job.name == 'burger' then
            ...
            TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, 'You fired '..TargetPlayer.PlayerData.charinfo.firstname..' '..TargetPlayer.PlayerData.charinfo.lastname..' from Burgershot', 'success')
