$(document).on('click', '.accept-button', function(e) {
    ...
    $.post('http://dream-burgershot/Click', JSON.stringify({}))
    if (PriceAmount != '' && PriceAmount != undefined && NoteAmount != '' && NoteAmount != undefined) {
        $.post('http://dream-burgershot/AddPrice', JSON.stringify({Price: PriceAmount, Note: NoteAmount}))
        $.post('http://dream-burgershot/CloseNui', JSON.stringify({}))
        ...
        $.post('http://dream-burgershot/ErrorClick', JSON.stringify({}))
    }
});
...
$(document).on('click', '.payment', function(e) {
    ...
    if (NumberId != null && Note!= null && Price != null) {
        $.post('http://dream-burgershot/PayReceipt', JSON.stringify({Price: Price, Note: Note, Id: NumberId}))
        $.post('http://dream-burgershot/CloseNui', JSON.stringify({}))
        $(".main-container").animate({"top": "-30vh"}, 250, function() {
            $('.items').html('');
            $('.main-container').css("display", "none");
