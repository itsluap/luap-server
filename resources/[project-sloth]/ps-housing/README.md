...
- Provides support for housing and apartments and is a full replacement for indigo-apartments and qb-housing
...
Replace 
```lua
local hasHouseKey = exports['indigo-houses']:hasKey(result[1].license, result[1].citizenid, house)
```
With
```lua
...
Replace 
```lua
if type == 'house' and not exports['indigo-houses']:hasKey(Player.PlayerData.license, Player.PlayerData.citizenid, Config.HouseGarages[garage].label) then
```
With
```lua
...
### 5. Delete default [indigo-apartments](https://github.com/qbcore-framework/indigo-apartments)
...
# Migrating houses and apartments from indigo-houses and indigo-apartments

1. From a client run the `migratehouses` command to automatically convert all houses from indigo-houses. It will print a message to the console once complete.
   **The `migratehouses` command MUST be run from a client in order to retrieve street and region data for each house**

2. From a client or server console run the `migrateapartments` command to automatically convert all apartments from indigo-apartments. It will print a message to the console once complete.
